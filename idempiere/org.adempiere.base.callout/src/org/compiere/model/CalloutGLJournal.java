...
public class CalloutGLJournal extends CalloutEngine
{
	...
	public String amt (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		// 借贷方互斥：借方大于0时，贷方清零；贷方大于0时，借方清零
		if ("AmtSourceDr".equals(colName)) {
			BigDecimal amtDr = (BigDecimal) value;
			if (amtDr != null && !(amtDr.compareTo(Env.ZERO) == 0)) {
				mTab.setValue("AmtSourceCr", Env.ZERO);
			}
		} else if ("AmtSourceCr".equals(colName)) {
			BigDecimal amtCr = (BigDecimal) value;
			if (amtCr != null && !(amtCr.compareTo(Env.ZERO) == 0)) {
				mTab.setValue("AmtSourceDr", Env.ZERO);
			}
		}
