...
public class CalloutOrder extends CalloutEngine
{
		...
		try
		{
			...
			if (rs.next())		//	we found document type
			{
				...
				if (DocSubTypeSO.equals(MOrder.DocSubTypeSO_POS))
					...
					mTab.setValue("PaymentRule", X_C_Order.PAYMENTRULE_DirectDeposit);
	...
	public String bPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		String sql = "SELECT p.AD_Language,p.C_PaymentTerm_ID,"
			...
			//+ " p.SO_CreditLimit, p.SO_CreditLimit-p.SO_CreditUsed AS CreditAvailable,"
				+ " (p.SO_CreditLimit"
				+ "  + CASE WHEN p.SOCreditStatus='A'"
				+ "         AND p.TempCreditLimit IS NOT NULL AND p.TempCreditLimit > 0"
				+ "         AND (p.TempCreditLimitValidFrom IS NULL OR CURRENT_DATE >= p.TempCreditLimitValidFrom)"
				+ "         AND (p.TempCreditLimitValidTo   IS NULL OR CURRENT_DATE <= p.TempCreditLimitValidTo)"
				+ "    THEN p.TempCreditLimit ELSE 0 END) AS EffectiveCreditLimit,"
				+ " (p.SO_CreditLimit"
				+ "  + CASE WHEN p.SOCreditStatus='A'"
				+ "         AND p.TempCreditLimit IS NOT NULL AND p.TempCreditLimit > 0"
				+ "         AND (p.TempCreditLimitValidFrom IS NULL OR CURRENT_DATE >= p.TempCreditLimitValidFrom)"
				+ "         AND (p.TempCreditLimitValidTo   IS NULL OR CURRENT_DATE <= p.TempCreditLimitValidTo)"
				+ "    THEN p.TempCreditLimit ELSE 0 END) - p.SO_CreditUsed AS CreditAvailable,"
		...
		try
		{
			...
			if (rs.next())
			{
				...
				if (IsSOTrx)
				{
					double CreditLimit = rs.getDouble("EffectiveCreditLimit");
					String soCreditStatus = rs.getString("SOCreditStatus");
					if (CreditLimit != 0 || MBPartner.SOCREDITSTATUS_CustomCreditCheck.equals(soCreditStatus))
					{
	...
	public String amt (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		//明显加入MOQ价格逻辑，勾选【是否MOQ价格】后，单价=总价；不勾选按原本计算模式，单价*数量=总价
		Boolean isMOQ = (Boolean) mTab.getValue("IsMOQ");
		BigDecimal LineNetAmt = (isMOQ != null && isMOQ)
				? PriceEntered
				: QtyEntered.multiply(PriceEntered);
