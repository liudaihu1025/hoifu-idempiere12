...
public class DrillReportCtl {
	...
	private String mapColumnName(String columnName) {
		if ("C_ElementValue_ID".equals(columnName)) {
			return "Account_ID";
		}
		return columnName;
	}
...
//			MProcessDrillRule[] processDrillRules = MProcessDrillRule.getByColumnName(Env.getCtx(), m_ColumnName, null);
			...
			String searchColumnName = mapColumnName(m_ColumnName);
			MProcessDrillRule[] processDrillRules = MProcessDrillRule.getByColumnName(Env.getCtx(), searchColumnName,
					null);
...
//			if(processPara.getColumnName().equals(m_ColumnName))
			...
			if (processPara.getColumnName().equals(mapColumnName(m_ColumnName)))
			{
				if(m_Value == null) {
					if (log.isLoggable(Level.FINE)) log.fine(sPara.getColumnName() + " - empty");
					continue;
				}
				iPara.setParameter(DisplayType.isID(sPara.getDisplayType()) ? new BigDecimal(String.valueOf(m_Value))
	...
	private Object parseVariable(MProcessDrillRulePara sPara, String variable) {
				...
				&& variable.indexOf('@') != variable.lastIndexOf('@'))	//	we have a variable / BF [1926032]
		{
			...
			// 从当前窗口上下文获取行级参数
			String env = Env.getContext(Env.getCtx(), m_WindowNo, columnName, true);
			// 假设行数据没有期间，从全局上下文取期间钻取参数
			if ((env == null || env.length() == 0) && "C_Period_ID".equals(columnName))
