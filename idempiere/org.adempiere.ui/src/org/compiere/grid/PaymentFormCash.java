...
public abstract class PaymentFormCash extends PaymentForm {
	...
	protected Object paymentCategoryValue = null;
	...
	public boolean save(int C_BankAccount_ID, int C_CashBook_ID, Timestamp dateAcct, BigDecimal amount, String trxName)
	{
		...
		if (m_Cash_As_Payment)
		{
			...
			// 添加 PaymentCategory 保存逻辑
			if (paymentCategoryValue != null) {
				m_mPayment.set_CustomColumn("PaymentCategory", paymentCategoryValue);
			}

			// 从发票复制字段值
			if (invoice != null) {
				if (invoice.get_Value("R_Info") != null) {
					m_mPayment.set_CustomColumn("R_Info", invoice.get_Value("R_Info"));
				}
				if (invoice.getDescription() != null) {
					m_mPayment.setDescription(invoice.getDescription());
				}
			}
