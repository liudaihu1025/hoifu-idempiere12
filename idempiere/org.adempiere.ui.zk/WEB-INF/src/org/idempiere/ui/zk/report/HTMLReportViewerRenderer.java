...
public class HTMLReportViewerRenderer implements IReportViewerRenderer {
	...
	public AMedia renderMedia(ZkReportViewer viewer, boolean export) {
		...
		try {
	        ...
	        // 获取当前流程ID  
	        String processID = reportEngine.getPrintInfo() != null ?   
	            String.valueOf(reportEngine.getPrintInfo().getAD_Process_ID()) : "";  
			HTMLReportRendererConfiguration config = new HTMLReportRendererConfiguration()
					...
					.setOutputFile(file)
					.setExport(export)
					.setExtension(new HTMLExtension(contextPath, "rp", viewer.getUuid(), processID))
