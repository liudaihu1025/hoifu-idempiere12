...
import org.adempiere.impexp.CSVPostProcessor;
...
public class CSVReportViewerRenderer implements IReportViewerRenderer {
	...
	public AMedia renderMedia(ZkReportViewer viewer, boolean export) {
		...
		try {
			...
			if (MSysConfig.getBooleanValue(MSysConfig.EXPORT_INCLUDE_REFERENCE_FIELDS, false,
				...
				file = applyCSVPostProcessing(file);
			}
	...
	/**
	 * 应用CSV后处理，添加参考字段
	 * @Title: applyCSVPostProcessing
	 * @param originalCSV
	 * @return
	 * @return File
	 */
	private File applyCSVPostProcessing(File originalCSV) {
		try {
			CSVPostProcessor processor = new CSVPostProcessor();
			File processedFile = processor.processCSV(originalCSV);
			return processedFile;
		} catch (Exception e) {
			log.log(Level.SEVERE, "Failed to apply CSV post processing", e);
			return originalCSV; // 返回原始文件
		}
	}
}
