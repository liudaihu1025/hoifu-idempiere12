...
ContextMenuListener, IZoomableEditor
{
	...
	protected void actionQuickEntry (boolean newRecord)
	{
		...
		if (!newRecord)
		{
			// value 为空说明当前字段未选择任何记录，无法进入编辑模式
			if (value == null) {
				Messagebox.show(Msg.getMsg(Env.getCtx(), "AccessCannotUpdate", true), 
		...
		// 新建时，如果有选中的 ID，从数据库查询 Name 字段带入
		if (newRecord && getValue() instanceof Integer) {
			int selectedId = (Integer) getValue();
			if (selectedId > 0) {
				try {
					// lookup.getColumnName() 格式为 "TableName.KeyColumn"
					String colName = lookup.getColumnName();
					int dotIdx = colName.indexOf(".");
					String tableName = dotIdx > 0 ? colName.substring(0, dotIdx)
							: colName.substring(0, colName.length() - 3);
					String name = DB.getSQLValueString(null,
							"SELECT Name FROM " + tableName + " WHERE " + tableName + "_ID = ?", selectedId);
					if (!Util.isEmpty(name, true)) {
						vqe.setFieldValue("Name", name);
					}
				} catch (Exception e) {
					logger.warning("获取选择记录名称异常：" + e.getMessage());
				}
			}
		}
