...
public class CSVImportAction implements EventListener<Event>
{
	...
	public void fileImport()
	{
		...
		if (theCSVImporter == null)
			throw new AdempiereException("找不到 CSV 导入器，请检查系统插件配置");
		...
		if (fTemplates.getItemCount() > 0) {
			...
			throw new AdempiereException("当前页签没有可用的导入模板，请联系管理员配置导入模板");
		}
	...
	private void importFile() {
		try {
			...
			// 添加这段代码来确保流支持mark/reset  
			if (!m_file_istream.markSupported()) {  
			    m_file_istream = new BufferedInputStream(m_file_istream);  
			} 
