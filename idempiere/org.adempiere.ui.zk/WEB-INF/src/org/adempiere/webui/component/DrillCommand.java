...
//		Events.postEvent(new DrillEvent(request.getCommand(), comp, new DrillEvent.DrillData(query, columnName, code, displayValue, data )));
		...
		Object rowData = null;
		if (data.size() >= 5) {
			rowData = data.get(4); // 第5个元素是行数据
		}
		Events.postEvent(new DrillEvent(request.getCommand(), comp, new DrillEvent.DrillData(query, columnName, code, displayValue, data, rowData )));
