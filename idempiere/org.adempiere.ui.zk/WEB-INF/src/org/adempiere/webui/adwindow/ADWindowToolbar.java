...
{	
    ...
    private ToolBarButton createButton(String name, String image, String tooltip)
    {
        ...
        // 读取ADWINDOW_TOOLBAR_STYLE系统配置  
        String buttonStyle = MSysConfig.getValue("ADWINDOW_TOOLBAR_STYLE","I");  
        boolean withText = buttonStyle.indexOf("T") >= 0;  
        if (withText) {  
			// 尝试获取 valueText
			String valueText = Msg.getMsg(Env.getCtx(), tooltip, true);
			// 若未找到，则回退到完整提示文本
			if (valueText == null || valueText.isEmpty()) {
				valueText = tooltipText;
			}
			btn.setLabel(valueText);
            LayoutUtils.addSclass("toolbarbutton-with-text", btn);  
        }  
	...
	private void doOnClick(Event event) {
		...
		if (event.getTarget() == fQueryName) {
			...
			// 新增：判断是否为凭证窗口的新增按钮
			if ("BtnNew".equals(compName) && windowContent != null) {
			    GridTab activeTab = windowContent.getActiveGridTab();
			    if (activeTab != null) {
			        String tableName = activeTab.getTableName();
			        // 在监听器中查找 AbstractADWindowContent 实例并执行对应操作
			        for (ToolbarListener tListener : listeners) {
			            if (tListener instanceof AbstractADWindowContent) {
			                AbstractADWindowContent content = (AbstractADWindowContent) tListener;
			                if ("GL_Journal".equals(tableName)) {
			                    content.onVoucherQuickEntry(); return;
			                } else if ("GL_JournalLine".equals(tableName)) {
			                    content.onQuickForm(true); return;
			                }
			            }
			        }
			    }
