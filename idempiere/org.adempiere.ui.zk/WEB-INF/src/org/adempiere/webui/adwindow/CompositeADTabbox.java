...
public class CompositeADTabbox extends AbstractADTabbox
{
    ...
    protected void onEditDetail(int row, boolean formView) {
	    ...
	    // 只对GL_Journal表自动打开Quick Form  
		if (headerTab.getGridTab().getAD_Table_ID() == MJournalLine.Table_ID) { // GL_Journal的Table ID
	        adWindowPanel.onQuickForm(true); // 设置为false，取消时不返回父记录  
	    }  
	}
