...
public class QuickGridView extends Vbox
	...
	/**
	 * 设置弹性拉伸模式
	 * @param stretch true 启用：表格宽度填满父容器，列宽按原有比例拉伸
	 */
	public void setStretchMode(boolean stretch) {
	    if (this.stretchMode == stretch) return;
	    this.stretchMode = stretch;
	    if (stretch) {
	        applyStretchMode();
	    } else {
	        // 如果需要恢复原有行为，可在此实现重置逻辑，但通常不需要
	    }
	}
	 ...
	 * 应用拉伸模式的具体设置
	 */
	private void applyStretchMode() {
	    if (listbox == null) return;
	    // 让 Grid 本身填满父容器
	    listbox.setHflex("1");
	    // 开启 span 模式：当 Grid 宽度大于各列原始宽度之和时，按比例拉伸列
	    listbox.setSpan("true");
	    
	    Columns columns = (Columns) listbox.getColumns();
	    if (columns == null) return;
	    // 清除所有数据列（跳过第一列复选框）的 hflex，避免与 span 冲突
	    List<Component> columnList = columns.getChildren();
	    for (int i = 1; i < columnList.size(); i++) {
	        Column col = (Column) columnList.get(i);
	        if (col.getHflex() != null && !col.getHflex().equals("")) {
	            col.setHflex(null);
	        }
	    }
	    listbox.invalidate();
	...
	/**
	 * 当刷新或重建表格后，重新应用拉伸模式（如果启用）
	 */
	 private void applyStretchModeIfNeeded() {
	    if (stretchMode) {
	        applyStretchMode();
	    }
	}
