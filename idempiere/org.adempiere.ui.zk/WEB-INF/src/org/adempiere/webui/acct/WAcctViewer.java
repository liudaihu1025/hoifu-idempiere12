...
import org.adempiere.webui.component.VoucherMergeRenderer;
...
public class WAcctViewer extends Window implements EventListener<Event>
{
	...
	/** 是否显示反冲 */
	private Checkbox displayReversed = new Checkbox();
	// === 凭证相关 UI 组件 ===
	private Listbox selVoucherType = new Listbox();
	private Label lVoucherType = new Label();
	private Checkbox displayVoucher = new Checkbox(); // 显示凭证信息
	private Label lSeqNoFrom = new Label();
	private Label lSeqNoTo = new Label();
	private Textbox fSeqNoFrom = new Textbox();
	private Textbox fSeqNoTo = new Textbox();
	private Label lVoucherDocNo = new Label();
	private Textbox fVoucherDocNo = new Textbox();
	private Label lVoucherDesc = new Label();
	private Textbox fVoucherDesc = new Textbox();
	...
	private void init() throws Exception
	{
		...
		// === 凭证搜索字段 ===
		lVoucherType.setValue("凭证字");
		selVoucherType.setMold("select");

		// 通过引用名称获取引用ID
		int voucherTypeRefId = 0;
		String sql = "SELECT AD_Reference_ID FROM AD_Reference WHERE Name = ?";
		voucherTypeRefId = DB.getSQLValue(null, sql, "Gl_VoucherType");
		if (voucherTypeRefId > 0) {
			// 获取引用列表数据
			ValueNamePair[] voucherTypes = MRefList.getList(Env.getCtx(), voucherTypeRefId, true, null, null);
			for (ValueNamePair vnp : voucherTypes) {
				selVoucherType.appendItem(vnp.getName(), vnp.getValue());
			}
		} else {
			// 如果找不到引用，添加默认选项
			selVoucherType.appendItem("", "");
		}
		row = rows.newRow();
		row.appendChild(lVoucherType);
		row.appendChild(selVoucherType);
		ZKUpdateUtil.setHflex(selVoucherType, "1");

		lSeqNoFrom.setValue("凭证号");
		lSeqNoTo.setValue("凭证号到");
		lVoucherDocNo.setValue("凭证编号");
		lVoucherDesc.setValue("摘要");

		// 凭证号范围
		row = rows.newRow();
		row.appendChild(lSeqNoFrom);
		Hlayout seqNoLayout = new Hlayout();
		seqNoLayout.appendChild(fSeqNoFrom);
		seqNoLayout.appendChild(new Label(" - "));
		seqNoLayout.appendChild(fSeqNoTo);
		row.appendChild(seqNoLayout);
		ZKUpdateUtil.setHflex(fSeqNoFrom, "1");
		ZKUpdateUtil.setHflex(fSeqNoTo, "1");

		// 凭证编号
		row = rows.newRow();
		row.appendChild(lVoucherDocNo);
		row.appendChild(fVoucherDocNo);
		ZKUpdateUtil.setHflex(fVoucherDocNo, "1");

		// 摘要（模糊查询）
		row = rows.newRow();
		row.appendChild(lVoucherDesc);
		row.appendChild(fVoucherDesc);
		ZKUpdateUtil.setHflex(fVoucherDesc, "1");

		// 凭证号和凭证编号互斥事件
		fSeqNoFrom.addEventListener(Events.ON_CHANGE, this);
		fSeqNoTo.addEventListener(Events.ON_CHANGE, this);
		fVoucherDocNo.addEventListener(Events.ON_CHANGE, this);
		...
		//是否显示反冲
		displayReversed.setLabel("显示反冲");
		displayReversed.addEventListener(Events.ON_CHECK, this);
		displayPanel.appendChild(displayReversed);
		
		// 显示凭证信息
		displayVoucher.setLabel("显示凭证");
		displayVoucher.setChecked(true); // 设置默认勾选
		displayVoucher.addEventListener(Events.ON_CHECK, this);
		displayPanel.appendChild(displayVoucher);
	...
	private void dynInit (int AD_Table_ID, int Record_ID)
	{
		...
		for (int i = 0; i < selPostingType.getItemCount(); i++) {
			Listitem item = selPostingType.getItemAtIndex(i);
			ValueNamePair vp = (ValueNamePair) item.getValue();
			if (vp != null && "A".equals(vp.getValue())) {
				selPostingType.setSelectedIndex(i);
				break;
			}
		}
		...
		if (loginOrgId > 0) {
			for (int i = 0; i < selOrg.getItemCount(); i++) {
				Listitem item = selOrg.getItemAtIndex(i);
				KeyNamePair kp = (KeyNamePair) item.getValue();
				if (kp != null && kp.getKey() == loginOrgId) {
		...
		// 默认日期：当月首日 00:00:00 ~ 当月末日 23:59:59.999
		Timestamp now = new Timestamp(System.currentTimeMillis());
		selDateFrom.setValue(TimeUtil.getMonthFirstDay(now));
		selDateTo.setValue(TimeUtil.getMonthLastDay(now));
		//  Document Select
	...
	public void onEvent(Event e) throws Exception
	{
		...
		//
		// 凭证号和凭证编号互斥
		else if (source == fSeqNoFrom || source == fSeqNoTo) {
			boolean hasSeqNo = (fSeqNoFrom.getText() != null && fSeqNoFrom.getText().trim().length() > 0)
					|| (fSeqNoTo.getText() != null && fSeqNoTo.getText().trim().length() > 0);
			fVoucherDocNo.setReadonly(hasSeqNo);
			if (hasSeqNo)
				fVoucherDocNo.setText("");
		} else if (source == fVoucherDocNo) {
			boolean hasDocNo = fVoucherDocNo.getText() != null && fVoucherDocNo.getText().trim().length() > 0;
			fSeqNoFrom.setReadonly(hasDocNo);
			fSeqNoTo.setReadonly(hasDocNo);
			if (hasDocNo) {
				fSeqNoFrom.setText("");
				fSeqNoTo.setText("");
			}
	...
	private void actionAcctSchema()
	{
		...
		// 凭证号排序选项
		sortAddItem(new ValueNamePair(
				"(SELECT gv.SeqNo FROM Gl_Voucher gv WHERE gv.Gl_Voucher_ID=" + RModel.TABLE_ALIAS + ".Gl_Voucher_ID)",
				"凭证号"));
...
//				.append(", Record_ID=").append(m_data.Record_ID);
			if (m_data.AD_Table_ID == 0)
				return; // 至少要选择单据类型

			// Record_ID 可以为 0，表示查询该类型的所有单据
			if (m_data.Record_ID != 0) {
				para.append(", AD_Table_ID=").append(m_data.AD_Table_ID).append(", Record_ID=")
						.append(m_data.Record_ID);
			} else {
				para.append(", AD_Table_ID=").append(m_data.AD_Table_ID);
			}
		}
		...
		/** 是否显示反冲 */
		m_data.displayReversed = displayReversed.isChecked();
		// 凭证显示选项
		m_data.displayVoucher = displayVoucher.isChecked();

		// 凭证搜索参数
		// 凭证字
		Listitem voucherTypeItem = selVoucherType.getSelectedItem();
		m_data.VoucherType = (voucherTypeItem != null && voucherTypeItem.getValue() != null)
				? (String) voucherTypeItem.getValue()
				: null;

		String seqFromStr = fSeqNoFrom.getText();
		String seqToStr = fSeqNoTo.getText();
		m_data.SeqNoFrom = (seqFromStr != null && seqFromStr.trim().length() > 0) ? Integer.valueOf(seqFromStr.trim())
				: null;
		m_data.SeqNoTo = (seqToStr != null && seqToStr.trim().length() > 0) ? Integer.valueOf(seqToStr.trim()) : null;
		m_data.VoucherDocumentNo = fVoucherDocNo.getText();
		m_data.VoucherDescription = fVoucherDesc.getText();
		...
		// 创建并配置渲染器
		VoucherMergeRenderer voucherRenderer = new VoucherMergeRenderer();
		voucherRenderer.setRModel(m_rmodel);           // 必须设置
		voucherRenderer.setTableModel(listmodeltable); // 必须设置
		table.setItemRenderer(voucherRenderer);
	...
	private void actionZoomFactAcct() {
		...
		// 1. 获取 Gl_Voucher 表的 AD_Table_ID
		int glVoucherTableID = MTable.getTable_ID("Gl_Voucher");

		// 2. 单独查询 Fact_Acct.Gl_Voucher_ID（不抛异常，出错返回 -1）
		if (glVoucherTableID > 0) {
			int glVoucherID = DB.getSQLValue(null,
					"SELECT Gl_Voucher_ID FROM Fact_Acct WHERE Fact_Acct_ID=?",
					fact_acct_ID);
			if (glVoucherID > 0) {
				AEnv.zoom(glVoucherTableID, glVoucherID);
				return;
			}
		}
