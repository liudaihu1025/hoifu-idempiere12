...
public class WQuickEntry extends AbstractWQuickEntry implements EventListener<Event>, ValueChangeListener
{
	...
	protected boolean actionSave()
	{
		...
		for (int idxt = 0; idxt < quickTabs.size(); idxt++) {
			...
			for (int idxf = 0; idxf < quickFields.size(); idxf++) {
				...
				// 已存在记录时，跳过不可更新的列（避免 ColumnReadonly 异常）
	...
	public void setFieldValue(String columnName, Object value) {
		for (int i = 0; i < quickFields.size(); i++) {
			if (quickFields.get(i).getColumnName().equals(columnName)) {
				quickEditors.get(i).setValue(value);
				quickEditors.get(i).updateStyle();
				break;
			}
		}
	}
