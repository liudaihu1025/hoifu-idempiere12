...
public class WPaymentFormCash extends PaymentFormCash {
	...
	private Label bPaymentCategoryLabel = new Label();
	private WTableDirEditor bPaymentCategoryField;
	...
	public void init() {
		...
		// 初始化字段
		bPaymentCategoryLabel.setText(Msg.translate(Env.getCtx(), "PaymentCategory"));
		bPaymentCategoryField = new WTableDirEditor("PaymentCategory", true, false, true, lookup);
		...
		// 添加PaymentCategory字段
		row = rows.newRow();
		row.appendChild(bPaymentCategoryLabel.rightAlign());
		row.appendChild(bPaymentCategoryField.getComponent());
	}
	...
	public boolean checkMandatory() {
		...
		Object paymentCategoryValue = bPaymentCategoryField.getValue();
		if (paymentCategoryValue == null || paymentCategoryValue.toString().trim().isEmpty()) {
			Dialog.error(getWindowNo(), "FillMandatory", bPaymentCategoryLabel.getValue());
			dataOK = false;
		}
	...
	public boolean saveChangesInTrx(final String trxName) {
		...
		if (bPaymentCategoryField != null && bPaymentCategoryField.getValue() != null) {
			paymentCategoryValue = bPaymentCategoryField.getValue();
		}
