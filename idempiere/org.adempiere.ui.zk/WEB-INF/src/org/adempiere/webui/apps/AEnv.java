...
public final class AEnv
{
	...
	int AD_Table_ID = MTable.getTable_ID(data.getQuery().getTableName());
		if (AD_Table_ID > 0) {
			...
			// 检查是否配置为直接跳转报表或窗口
			DrillConfig config = getDrillConfig(processID, data.getColumnName());
			if (config != null && config.matchesColumn(data.getColumnName())) {
				if (config.isDirectDrillReport()) {
					if (executeDirectDrillWithoutDialog(data, windowNo, processID))
						return;
				} else if (config.isDirectZoomWindow()) {
					if (executeDirectZoomWithoutDialog(data, windowNo))
						return;
				}
			}
	 ...
	 * 获取钻取简化配置
	 * 
	 * @Title: getDrillConfig
	 * @param processID
	 * @param columnName
	 * @return
	 * @return DrillConfig
	 */
	public static DrillConfig getDrillConfig(int processID, String columnName) {
		String sql = "SELECT DrillType, Drill_ColumnName FROM AD_ReportDrill_Config WHERE AD_Process_ID = ? ";
		if (!Util.isEmpty(columnName)) {
			sql += " AND (Drill_ColumnName = ? OR Drill_ColumnName IS NULL) ";
		}
		sql +=" AND IsActive = 'Y' ORDER BY Drill_ColumnName DESC, Updated DESC";
		...
		try {
			...
			if (rs.next()) {
				String drillType = rs.getString("DrillType");
				String drillColumn = rs.getString("Drill_ColumnName");
				return new DrillConfig(drillType, drillColumn);
			}
		} catch (SQLException e) {
			log.log(Level.SEVERE, "Failed to query drill config", e);
		} finally {
			DB.close(rs, pstmt);
		}
		return null;
	 ...
	 * 钻取简化配置内部类
	 * 
	 * @ClassName: DrillConfig
	 * @author ldh
	 * @date 2026年5月4日
	 */
	public static class DrillConfig {
		public final String drillType;
		public final String drillColumn;

		DrillConfig(String drillType, String drillColumn) {
			this.drillType = drillType;
			this.drillColumn = drillColumn;
		}

		public boolean isDirectDrillReport() {
			return "R".equals(drillType);
		}

		public boolean isDirectZoomWindow() {
			return "W".equals(drillType);
		}

		public boolean matchesColumn(String columnName) {
			return Util.isEmpty(drillColumn) || drillColumn.equals(columnName);
		}
	}
	 ...
	 * 直接执行钻取窗口，不显示任何选择界面
	 * 
	 * @Title: executeDirectZoomWithoutDialog
	 * @param data
	 * @param windowNo
	 * @return
	 * @return boolean
	 */
	private static boolean executeDirectZoomWithoutDialog(DrillData data, int windowNo) {
		try {
			MQuery query = data.getQuery();
			if (query != null) {
				// 创建最简单的查询，只使用主键
				MQuery simpleQuery = createSimpleKeyQuery(query);

				// 直接调用系统zoom方法进行跳转
				AEnv.zoom(simpleQuery);
				return true;
			}
		} catch (Exception e) {
			log.log(Level.SEVERE, "执行直接跳转窗口失败: " + e.getMessage(), e);
		}
		return false;
	}
	...
	/**
	 * 创建最简单的主键查询
	 * @Title: createSimpleKeyQuery
	 * @param originalQuery
	 * @return
	 * @return MQuery
	 */
	private static MQuery createSimpleKeyQuery(MQuery originalQuery) {
		String tableName = originalQuery.getTableName();
		MQuery simpleQuery = new MQuery(tableName);

		// 优先使用zoom信息
		if (originalQuery.getZoomColumnName() != null && originalQuery.getZoomValue() != null) {
			simpleQuery.addRestriction(originalQuery.getZoomColumnName(), MQuery.EQUAL, originalQuery.getZoomValue());
			return simpleQuery;
		}

		// 否则从DrillData中获取主键信息
		Object keyValue = getKeyValueFromQuery(originalQuery);
		if (keyValue != null) {
			String keyColumn = tableName + "_ID";
			simpleQuery.addRestriction(keyColumn, MQuery.EQUAL, keyValue);
			simpleQuery.setZoomValue(keyValue);
			simpleQuery.setZoomColumnName(keyColumn);
			simpleQuery.setZoomTableName(tableName);
		}

		simpleQuery.setRecordCount(1);
		return simpleQuery;
	}

	/**
	 * 从查询中提取主键值
	 * @Title: getKeyValueFromQuery
	 * @param query
	 * @return
	 * @return Object
	 */
	private static Object getKeyValueFromQuery(MQuery query) {
		// 遍历所有限制条件查找主键值
		for (int i = 0; i < query.getRestrictionCount(); i++) {
			String columnName = query.getColumnName(i);
			if (columnName != null && columnName.endsWith("_ID")) {
				return query.getCode(i);
			}
		}
		return null;
	}
	 ...
	 * 直接执行钻取报表，不显示任何选择界面
	 * 
	 * @Title: executeDirectDrillWithoutDialog
	 * @param data
	 * @param windowNo
	 * @param processID
	 * @return void
	 */
	private static boolean executeDirectDrillWithoutDialog(DrillData data, int windowNo, int processID) {
		try {
			DrillReportCtl drillReportCtl = new DrillReportCtl(data.getQuery().getTableName(), data.getQuery(),
					data.getColumnName(), data.getValue(), data.getDisplayValue(), windowNo, processID);

			// 用 Stream 扁平化所有 Rule，找优先级最高（值最小）的那条钻取配置
			Optional<MProcessDrillRule> bestRule = drillReportCtl.getDrillProcesRules().values().stream()
					.flatMap(Arrays::stream)
					.map(rule -> MProcessDrillRule.get(Env.getCtx(), rule.getKey()))
					.filter(Objects::nonNull)
					.min(Comparator.comparingInt((MProcessDrillRule r) -> {
						int priority = r.get_ValueAsInt("Priority"); // 替换为实际字段名
						return priority == 0 ? Integer.MAX_VALUE : priority;
					}).thenComparing(Comparator.comparingInt(MProcessDrillRule::getAD_Process_DrillRule_ID).reversed()));

			if (bestRule.isPresent()) {
				ProcessInfo pi = drillReportCtl.getDrillProcessProcessInfo(bestRule.get().getAD_Process_DrillRule_ID(), 0);
				...
				// 保存参数到数据库
				fillParameter(instance, buildParameterString(pi.getParameter()));

                // 静默执行不弹窗
				pi.setShowHelp(MProcess.SHOWHELP_RunSilently_TakeDefaults);

                // 直接执行进程，绕过参数面板验证
				WProcessCtl worker = new WProcessCtl(null, windowNo, pi, null);
				worker.run();
				return true;
			}
		} catch (Exception e) {
			log.log(Level.SEVERE, "Failed to execute direct drill without dialog", e);
			return false;
		}
		return false;
	}
	...
	/**
	 * 将 ProcessInfoParameter 数组转换为参数字符串
	 * 
	 * @param params ProcessInfoParameter 数组
	 * @return 格式化的参数字符串，如 "ColumnName=Value,ColumnName2=Value2"
	 */
	private static String buildParameterString(ProcessInfoParameter[] params) {
		...
		for (int i = 0; i < params.length; i++) {
			...
			if (paramValue != null) {
				...
				if (paramValue.toString().contains(",")) {
					paramString.append(paramName).append("=\"").append(paramValue.toString()).append("\"");
				} else {
					paramString.append(paramName).append("=").append(paramValue.toString());
				}
			}
		}
	 ...
	 */
	private static boolean fillParameter(MPInstance pInstance, String parameters) {	
			...
			for (int pi = 0; pi < processParams.length; pi++)
			{
				...
				for (String paramValue : variable.split(";")) {
						 ...
						 if (!Util.isEmpty(sql)) {
							 ...
							 try {
								 ...
								 if (rs.next()) {
									 ...
									 if (log.isLoggable(Level.INFO))
										 log.log(Level.INFO, "(AEnv " + iPara.getParameterName() + ") - no Result: " + sql);
								 }
							 }
							 catch (SQLException e) {
								 log.log(Level.WARNING, "(" + iPara.getParameterName() + ") " + sql, e);
							 }
							 finally{
								 DB.close(rs, stmt);
								 rs = null;
					 ...
					 if( DisplayType.isText(iPara.getDisplayType())
						...
						if (log.isLoggable(Level.FINE)) log.fine(iPara.getParameterName() + " - empty string AEnv");
							break;
					}
	 ...
	 * @return display text
	 */
	private static String getDisplay(MPInstance i, MPInstancePara ip, Object value) {
		try {
			...
			if (pp != null) {
				...
				try
				{
					...
					if (rs.next()) {
						...
						if (rs.next())
							log.log(Level.SEVERE, "AEnv Error while displaying parameter for embedded report - Not unique (first returned) for SQL=" + mli.QueryDirect);
					}
				}
				catch (Exception e) {
					log.log(Level.SEVERE, "AEnv Error while displaying parameter for embedded report - " + mli.KeyColumn + ": SQL=" + mli.QueryDirect + " : " + e);
				}
				finally {
					DB.close(rs, pstmt);
					rs = null;
			...
			log.log(Level.WARNING, "AEnv Failed to retrieve data to display for embedded report " + MProcess.get(i.getAD_Process_ID()).getName() + " : " + ip.getParameterName(), e);
		}
	 ...
	 * @return display text
	 */
	private static String getMultiSelectionDisplay(MPInstance i, MPInstancePara ip, String values) {
		String returnValue = "";
