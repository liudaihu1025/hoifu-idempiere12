...
public class WPaymentFormCheck extends PaymentFormCheck implements EventListener<Event> {
	...
	private Label sPaymentCategoryLabel = new Label();
	private WTableDirEditor sPaymentCategoryField;
	...
	protected void init() {
		...
		// 创建 PaymentCategory Lookup
		int columnID = MColumn.getColumn_ID(MPayment.Table_Name, "PaymentCategory");  
		MLookup lookup = MLookupFactory.get(Env.getCtx(), getWindowNo(), 0, columnID, DisplayType.List);  
		  
		// 初始化字段  
		sPaymentCategoryLabel.setText(Msg.translate(Env.getCtx(), "PaymentCategory"));  
		sPaymentCategoryField = new WTableDirEditor("PaymentCategory", true, false, true, lookup);
		...
		column = new Column();
		columns.appendChild(column);
		ZKUpdateUtil.setWidth(column, "75%");
		...
		// 添加 PaymentCategory 字段
		row = rows.newRow();
		row.appendChild(sPaymentCategoryLabel.rightAlign());
		row.appendChild(sPaymentCategoryField.getComponent());
	...
	public boolean checkMandatory() {
		...
		// Validate PaymentCategory field
		Object paymentCategoryValue = sPaymentCategoryField.getValue();
		if (paymentCategoryValue == null || paymentCategoryValue.toString().trim().isEmpty()) {
			Dialog.error(getWindowNo(), "FillMandatory", sPaymentCategoryLabel.getValue());
			dataOK = false;
		}
		//
	...
	public boolean saveChangesInTrx(final String trxName) {
		...
		// 设置 PaymentCategory 值到父类字段
		if (sPaymentCategoryField != null && sPaymentCategoryField.getValue() != null) {
			paymentCategoryValue = sPaymentCategoryField.getValue();
		}
