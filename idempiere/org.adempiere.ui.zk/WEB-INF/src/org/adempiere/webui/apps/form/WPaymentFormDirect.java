...
public abstract class WPaymentFormDirect extends PaymentFormDirect implements EventListener<Event> {
	...
	private Label tPaymentCategoryLabel = new Label();
	private WTableDirEditor tPaymentCategoryField;
	/**
	 * @param windowNo
	 * @param mTab
	...
	protected void init() {		
		...
		// 初始化 PaymentCategory 字段
		tPaymentCategoryLabel.setText(Msg.translate(Env.getCtx(), "PaymentCategory"));
		tPaymentCategoryField = new WTableDirEditor("PaymentCategory", true, false, true, lookup);
		...
		// 添加 PaymentCategory 字段行
		row = rows.newRow();
		row.appendChild(tPaymentCategoryLabel.rightAlign());
		row.appendChild(tPaymentCategoryField.getComponent());
	...
	public boolean checkMandatory() {
		...
		Object paymentCategoryValue = tPaymentCategoryField.getValue();
		if (paymentCategoryValue == null || paymentCategoryValue.toString().trim().isEmpty()) {
			Dialog.error(getWindowNo(), "FillMandatory", tPaymentCategoryLabel.getValue());
			dataOK = false;
		}
		//
	...
	public boolean saveChangesInTrx(final String trxName) {		
		...
		if (tPaymentCategoryField != null && tPaymentCategoryField.getValue() != null) {
			paymentCategoryValue = tPaymentCategoryField.getValue();
		}
