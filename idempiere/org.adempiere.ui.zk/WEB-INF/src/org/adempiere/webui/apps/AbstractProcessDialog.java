...
public abstract class AbstractProcessDialog extends Window implements IProcessUI, EventListener<Event>
{
	...
	private void setReportTypeAndPrintFormat(MPInstance instance)
	{
		...
		// 新增：当打印格式未设置时，使用PA_Report的默认打印格式
		if (fPrintFormat != null && fPrintFormat.getValue() == null && isFinancialReportProcess()) {
			int recordId = getProcessInfo().getRecord_ID();
			if (recordId > 0) {
				MReport report = new MReport(Env.getCtx(), recordId, null);
				if (report.getAD_PrintFormat_ID() > 0) {
					fPrintFormat.setValue(report.getAD_PrintFormat_ID());
					log.info("Set default print format from PA_Report: " + report.getAD_PrintFormat_ID());
				}
			}
		}
	...
	private boolean isFinancialReportProcess() {
		return m_AD_Process_ID == SystemIDs.PROCESS_RPT_FINREPORT;
	}
