...
import org.adempiere.impexp.ExcelPostProcessor;
...
//				exporter.doExport();
	...
	private void showExportFormatDialog() {  
	    ...
	    dialog.setTitle(Msg.getMsg(Env.getCtx(), "Export"));  
	    ...
	    Label label = new Label(Msg.getMsg(Env.getCtx(), "请选择导出格式"));  
	    Combobox formatCombo = new Combobox();  
	    formatCombo.appendItem("Excel (XLSX)", "xlsx");  
	    formatCombo.appendItem("PDF", "pdf");  
	    formatCombo.setSelectedIndex(0);  
	      
	    Button okButton = new Button(Msg.getMsg(Env.getCtx(), "确认"));  
	    Button cancelButton = new Button(Msg.getMsg(Env.getCtx(), "取消"));  
	      
	    Hlayout buttonLayout = new Hlayout();  
	    buttonLayout.appendChild(okButton);  
	    buttonLayout.appendChild(cancelButton);  
	      
	    layout.appendChild(label);  
	    layout.appendChild(formatCombo);  
	    layout.appendChild(buttonLayout);  
	    dialog.appendChild(layout);  
	      
	    okButton.addEventListener(Events.ON_CLICK, evt -> {  
	        String format = formatCombo.getSelectedItem().getValue();  
	        try {  
	            ...
	            Dialog.error(p_WindowNo, "Error", e.getMessage());  
	        }  
	    });  
	      
	    cancelButton.addEventListener(Events.ON_CLICK, evt -> dialog.detach());  
	      
	    dialog.setAttribute(Window.MODE_KEY, Window.MODE_HIGHLIGHTED);  
	    AEnv.showWindow(dialog);  
	}
	
	private class PdfExporter extends AbstractXLSXExporter {  
		...
		protected void setCurrentRow(int row)
		{
		    ...
		    {  
		        try  
		        {  
		            // Only call next() if we're not at the first row  
		            if (currentRow == -1) {  
		                // First time positioning - ResultSet should be before first  
	    ...
	    public void doExport() throws Exception {  
	        ...
	        File file = File.createTempFile(infoWindow.get_Translation("Name")+"_", ".pdf");  
	        ...
	        if(rowCount > 0) {  
	            ...
	            try {  
	                ...
	                trx.setDisplayName(getClass().getName()+"_exportPdf");  
	            ...
	            // 使用 WMediaOptions 模式替代直接下载  
	            AMedia media = new AMedia(file.getName(), null, "application/pdf", file, true);    
	            IMediaView view = Extensions.getMediaView("application/pdf", "pdf", ClientInfo.isMobile());  
	    ...
	    private void exportToPdf(File file) throws Exception {    
	    	    com.lowagie.text.Document document = new com.lowagie.text.Document(PageSize.A4.rotate(), 36, 36, 72, 72);      
	    	    ...
	    	    Paragraph title = new Paragraph(infoWindow.get_Translation("Name"), titleFont);      
	    	    title.setAlignment(Element.ALIGN_CENTER);      
	    	    document.add(title);      
	    	    document.add(Chunk.NEWLINE);      
	    	      
	    	    // 计算实际打印的列数  
	    	    int printableColCount = 0;  
	    	    ...
	    	    // 创建表格 - 使用实际打印的列数  
	    	    PdfPTable table = new PdfPTable(printableColCount);      
	    	    ...
	    	    for (int i = 0; i < getColumnCount(); i++) {      
	    	        if (isColumnPrinted(i)) {  
	    	            PdfPCell cell = new PdfPCell(new Phrase(getHeaderName(i), headerFont));      
	    	    ...
	    	    // 确保ResultSet在开始位置    
	    	    if (m_rs != null && !m_rs.isBeforeFirst()) {      
	    	        m_rs.beforeFirst();      
	    	    }      
	    	    ...
	    	    for (int row = 0; row < getRowCount(); row++) {        
	    	        ...
	    	        for (int col = 0; col < getColumnCount(); col++) {        
	    	            if (isColumnPrinted(col)) {      
	    	                ...
	    	                table.addCell(new Phrase(stringValue, dataFont));        
	    	            }      
	    	        }        
	    	    }      
	     ...
	     * 设置页眉
	     */
	    private void addHeader(com.lowagie.text.Document document) throws Exception {  
	        BaseFont baseFont = BaseFont.createFont("STSong-Light", "UniGB-UCS2-H", BaseFont.EMBEDDED);  
	        Font headerFont = new Font(baseFont, 20, Font.NORMAL);  
	          
	        Paragraph header = new Paragraph();  
	        header.add(new Chunk(getTenantName(), headerFont));  
	     ...
	     * 设置页脚
	     */
	    private void addFooter(com.lowagie.text.Document document, PdfWriter writer) throws Exception {  
	        BaseFont baseFont = BaseFont.createFont("STSong-Light", "UniGB-UCS2-H", BaseFont.EMBEDDED);  
	        Font footerFont = new Font(baseFont, 10, Font.NORMAL);  
	          
	        Paragraph footer = new Paragraph();  
	        footer.add(new Chunk("时间: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()), footerFont));  
