...
window.idempiere.drillAcross = function(cmpid, column, value, displayValue, processID, rowData){
	zAu.cmd0.showBusy(null);
	let widget = zk.Widget.$(cmpid);
	...
	// 构建事件数据数组
	let eventData = [column, value, displayValue, processID, rowData];


	let event = new zk.Event(widget, 'onDrillAcross', {data: eventData}, {toServer: true});
	zAu.send(event);
};
...
window.idempiere.showColumnMenu = function(doc, e, columnName, row, isUseFontIcons, processID) {
	...
	let isDirectDrill = e.target.getAttribute("isDirectDrill") || "";
	console.log("系统配置信息，isDirectDrill:" + isDirectDrill);
	if (isDirectDrill && "Y" === isDirectDrill) {
		// 获取行数据
		let rowData = null;
		let targetRow = e.target.closest('tr');
		if (targetRow) {
			rowData = targetRow.getAttribute('data-row-data');
		}
		console.log("获取行数据信息，rowData=" + rowData);
	    // 直接执行钻取，不显示菜单  
	    let componentId = e.target.getAttribute("componentId");  
	    let foreignColumnName = e.target.getAttribute("foreignColumnName");  
	    let value = e.target.getAttribute("value");  
	    let displayValue = e.target.getAttribute("displayValue");  
	      
	    parent.idempiere.drillAcross(componentId, foreignColumnName, value, displayValue, processID, rowData);  
	    return;  
	} 
	
	let d = idempiere.getMenu (doc, e.target.getAttribute ("componentId"), e.target.getAttribute ("foreignColumnName"), e.target.getAttribute ("value"), e.target.getAttribute ("displayValue"), isUseFontIcons, processID, e);
...
window.idempiere.getMenu = function(doc, componentId, foreignColumnName, value, displayValue, isUseFontIcons, processID, event){
	...
	if (componentId != null){	
		...
		reportDrillHref.setAttribute("onclick", "parent.idempiere.drillAcross('" + componentId + "','" + foreignColumnName + "','" + value + "','" + displayValue + "','" + processID + "', '" + rowData + "')");
