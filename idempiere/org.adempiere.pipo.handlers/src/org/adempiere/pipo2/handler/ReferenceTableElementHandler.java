...
public class ReferenceTableElementHandler extends AbstractElementHandler {
	...
	private void createReferenceTableBinding(PIPOContext ctx,
		...
		if (Objects.isNull(refTable)) {
			log.warning("AD_Reference_ID=[" + reference_ID + "]该引用的验证类型应该使用列表验证, 而不是表验证! AD_Ref_Table表没有找到该数据");
			throw new RuntimeException(
					"AD_Reference_ID=[" + reference_ID + "]该引用的验证类型应该使用列表验证, 而不是表验证! AD_Ref_Table表没有找到该数据");
		}
