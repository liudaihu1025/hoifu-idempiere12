	...
	public CreditStatus checkCreditStatus(String docAction)
	{
		...
		if (MInvoice.DOCACTION_Prepare.equals(docAction) && mInvoice.isSOTrx())
		{
					...
					|| (doc.getDocBaseType().equals(MDocType.DOCBASETYPE_ARInvoice) && mInvoice.getGrandTotal().signum() > 0))
			{
				...
				// 'A' status: credit check is done at order level, skip here
		...
		else if (MInvoice.DOCACTION_Complete.equals(docAction) || MInvoice.DOCACTION_Re_Activate.equals(docAction))
		{
			...
			if (mInvoice.isSOTrx())
			{
				...
				// 'A' status: SO_CreditUsed is managed at order level, skip update here
				if (!MBPartner.SOCREDITSTATUS_CustomCreditCheck.equals(bp.getSOCreditStatus()))
				{
					...
					if (log.isLoggable(Level.FINE))
						log.fine(", Credit=" + bp.getSO_CreditUsed() + "->" + newCreditAmt);
