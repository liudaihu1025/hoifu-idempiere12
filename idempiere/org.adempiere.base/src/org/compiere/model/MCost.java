...
public class MCost extends X_M_Cost implements ICostInfo
{
			...
			String trxName)
	{
		...
		// todo 问题：业务单据延迟处理导致成本数据异常，应增强回溯交易处理机制
		// 因为批量完成应付单对应的收货单是上个月的单据数据，距离如今较长时间（一个多月），期间产生了不少的业务单据和成本明细记录。导致应付单完成的时候创建成本明细和成本历史，
		// 创建成本明细时的当前数量取数逻辑：成本历史表小于应付单记账日期的变动后数量字段，按照M_CostHistory_ID倒序排序，取第一条。导致没取到最新的当前库存数量，发生数据异常
		// 检查是否为回溯交易（当前日期 - 记账日期 > 允许的天数）
//		boolean isBackDateTransaction = false;
//		if (dateAcct != null) {
//			Timestamp currentDate = new Timestamp(System.currentTimeMillis());
//			long daysDiff = (currentDate.getTime() - dateAcct.getTime()) / (24 * 60 * 60 * 1000);
//			// 如果超过7天，认为是回溯交易
//			isBackDateTransaction = daysDiff > 7;
//		}
		// 对于回溯交易的应付单完成，优先获取当前库存 && !isBackDateTransaction
		if (history == null && dateAcct != null) {	// get cost history record based on the account date
