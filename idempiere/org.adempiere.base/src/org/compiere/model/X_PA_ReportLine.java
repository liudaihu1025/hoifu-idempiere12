...
// @SQL=SELECT NVL(MAX(SeqNo),0)+10 AS DefaultValue FROM PA_ReportLine WHERE PA_ReportLineSet_ID=@PA_ReportLineSet_ID@
	...
	/** Formula Calculation = F */
	public static final String CALCULATIONTYPE_Formula = "F";
	...
	*/
	...
	/** Balance (Credit - Debit) = T */
	public static final String PAAMOUNTTYPE_BalanceCreditDebit = "T";
	...
	*/
	 ...
	 * Set Formula.
	 * 
	 * @param Formula Formula expression using @row references
	 */
	...
	public boolean isCalculationTypeFormula() {
		return CALCULATIONTYPE_Formula.equals(getCalculationType());
	}

	@Override
	protected boolean beforeSave(boolean newRecord) {
		// 检查同一报表行集内SeqNo唯一性
		String whereClause = "PA_ReportLineSet_ID = ? AND SeqNo = ? AND PA_ReportLine_ID != ?";
		int count = new Query(getCtx(), MReportLine.Table_Name, whereClause, get_TrxName())
				.setParameters(getPA_ReportLineSet_ID(), getSeqNo(), getPA_ReportLine_ID()).count();

		if (count > 0) {
			log.saveError("顺序号重复", "同一报表行集中已存在顺序号 " + getSeqNo());
			return false;
		}

		if (isCalculationTypeFormula()) {
			String formula = getFormula();

			// Check if formula is empty
			if (formula == null || formula.trim().isEmpty()) {
				log.saveError("必填字段", Msg.getElement(getCtx(), "Formula"));
				return false;
			}

			// Syntax validation
			if (!FormulaValidator.validateSyntax(formula)) {
				log.saveError("无效公式", FormulaValidator.getLastError());
				return false;
			}

			// For existing records, validate referenced lines exist
			if (!newRecord) {
				MReportLineSet reportLineSet = new MReportLineSet(getCtx(), getPA_ReportLineSet_ID(), get_TrxName());
				MReportLine[] lines = reportLineSet.getLiness();

				// 传入报表行集ID参数，确保只在同一报表行集内验证
				if (!FormulaValidator.validateRuntime(formula, lines, getPA_ReportLineSet_ID())) {
					log.saveError("无效公式", FormulaValidator.getLastError());
					return false;
				}
			}
