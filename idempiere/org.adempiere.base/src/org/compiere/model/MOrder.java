...
public class MOrder extends X_C_Order implements DocAction
{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		// 'A' status: after closeIt(), DocStatus is now 'CL' in DB.
		// Recalculate SO_CreditUsed from completed/closed unpaid orders to reflect
		// the adjusted GrandTotal (partial delivery may have reduced the order amount).
		if (!newRecord
				&& isSOTrx()
				&& is_ValueChanged(COLUMNNAME_DocStatus)
		...
		if (isSOTrx()) {
					...
					&& getGrandTotal().signum() > 0)
			{
				...
				if (grandTotal != null) {
					...
					if (newCreditUsed == null) newCreditUsed = Env.ZERO;
	...
	public boolean voidIt()
	{
		...
		// 'A' status: decrement SO_CreditUsed when order is voided
		// NOTE: must be before setTotalLines/setGrandTotal(ZERO) which clears the amount
		if (isSOTrx()) {
			MBPartner bp = new MBPartner(getCtx(), getBill_BPartner_ID(), get_TrxName());
			if (MBPartner.SOCREDITSTATUS_CustomCreditCheck.equals(bp.getSOCreditStatus())
					&& getGrandTotal().signum() > 0)
			{
				...
				if (grandTotal != null) {
					BigDecimal newCreditUsed = bp.getSO_CreditUsed();
					if (newCreditUsed == null) newCreditUsed = Env.ZERO;
					newCreditUsed = newCreditUsed.subtract(grandTotal);
					if (newCreditUsed.signum() < 0) newCreditUsed = Env.ZERO;
					bp.setSO_CreditUsed(newCreditUsed);
					bp.saveEx(get_TrxName());
				}
			}
