...
public class MSequence extends X_AD_Sequence
{
	...
	public static final String COLUMNNAME_CustomColumn = "CustomColumn";
	...
	public static String getDocumentNoFromSeq(MSequence seq, String trxName, PO po) {
		...
		// 新增自定义列处理
		boolean isUseCustomColumn = seq.isUseCustomColumn();
		String customColumn = seq.getCustomColumn();
		...
		if (isUseCustomColumn) {
			...
			// 如果获取失败或值为空，回退到标准序列模式
			if (customValue == null || customValue.isEmpty()) {
				isUseCustomColumn = false;
			}
		}

		StringBuilder selectSQL = new StringBuilder();

		if (isStartNewYear || isUseOrgLevel || isUseCustomColumn) {
			selectSQL.append("SELECT y.CurrentNext, s.CurrentNextSys ").append("FROM AD_Sequence_No y, AD_Sequence s ")
					.append("WHERE y.AD_Sequence_ID = s.AD_Sequence_ID ").append("AND s.AD_Sequence_ID = ? ");
			if (isUseOrgLevel)
				selectSQL.append("AND y.AD_Org_ID = ? ");
			if (isStartNewYear)
				selectSQL.append("AND y.CalendarYearMonth = ? ");
			if (isUseCustomColumn)
				selectSQL.append("AND y.CustomValue = ? ");
			selectSQL.append("AND s.IsActive='Y' AND s.IsTableID='N' AND s.IsAutoSequence='Y' ")
					.append("ORDER BY s.AD_Client_ID DESC");
		} else {
			selectSQL.append("SELECT s.CurrentNext, s.CurrentNextSys ").append("FROM AD_Sequence s ")
		...
		try
		{
			...
			pstmt = conn.prepareStatement(selectSQL.toString(), ResultSet.TYPE_FORWARD_ONLY,
					ResultSet.CONCUR_UPDATABLE);
			int index = 1;
			pstmt.setInt(index++, AD_Sequence_ID);
			if (isUseOrgLevel || isStartNewYear || isUseCustomColumn) {
				if (isUseOrgLevel)
					pstmt.setInt(index++, docOrg_ID);
				if (isStartNewYear)
					pstmt.setString(index++, calendarYearMonth);
				if (isUseCustomColumn)
					pstmt.setString(index++, customValue);
			}
			...
			if (rs.next())
			{
				...
				try
				{
					if (adempiereSys) {
						...
						String sql;
						if (isStartNewYear || isUseOrgLevel || isUseCustomColumn) {
							sql = "UPDATE AD_Sequence_No SET CurrentNext = CurrentNext + ? WHERE AD_Sequence_ID=?";
							if (isUseOrgLevel)
								sql += " AND AD_Org_ID=?";
							if (isStartNewYear)
								sql += " AND CalendarYearMonth=?";
							if (isUseCustomColumn)
								sql += " AND CustomValue=?";
						} else {
					...
					if (isStartNewYear)
						updateSQL.setString(idx++, calendarYearMonth);
					if (isUseCustomColumn)
						updateSQL.setString(idx++, customValue);
					updateSQL.executeUpdate();
				}
				finally
				{
			...
			{ // did not find sequence no
				...
				{	// create sequence (CurrentNo = StartNo + IncrementNo) for this year/month/org and return first number (=StartNo)
					...
					// 只在 isUseCustomColumn = true 时才设置 CustomValue
					if (isUseCustomColumn && customValue != null) {
						seqno.set_CustomColumn("CustomValue", customValue);
					}
	...
	public boolean isUseCustomColumn() {
		String customColumn = getCustomColumn();
		return customColumn != null && customColumn.length() > 0;
	}
