...
public class MImportTemplate extends X_AD_ImportTemplate implements ImmutablePOSupport
{
			...
			while ((line = reader.readLine()) != null) {
				if (firstLine == null) {
					...
					if (   firstLine.equals(getCSVHeader())
						...
						|| isChineseHeaderValid(firstLine)) {
	...
	// 新增方法：验证中文表头是否有效
	private boolean isChineseHeaderValid(String chineseHeader) {
		String[] headers = chineseHeader.split(",");
		String[] expectedHeaders = getCSVHeader().split(",");
		...
		for (int i = 0; i < headers.length; i++) {
			String chinese = headers[i].trim();
			String english = expectedHeaders[i].trim();

			// 尝试将中文列名翻译为英文进行验证
			String translated = translateChineseToEnglish(chinese);
			if (!english.equals(translated)) {
				return false;
			}
		}
		return true;
	...
	private String translateChineseToEnglish(String chineseText) {
		...
		return DB.getSQLValueString(null, sql, chineseText, adLanguage);
	}
