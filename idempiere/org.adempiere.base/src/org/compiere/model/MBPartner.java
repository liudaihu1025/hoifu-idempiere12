...
public class MBPartner extends X_C_BPartner implements ImmutablePOSupport
{
	...
	public void setTotalOpenBalance () {
		...
		if (SOCREDITSTATUS_CustomCreditCheck.equals(getSOCreditStatus())) {
			// 'A' status: TotalOpenBalance from invoices, SO_CreditUsed from completed unpaid orders

			// 1. TotalOpenBalance from invoices
			...
			try {
				balancePstmt = DB.prepareStatement(balanceSql, get_TrxName());
				balancePstmt.setInt(1, getC_BPartner_ID());
				balanceRs = balancePstmt.executeQuery();
				if (balanceRs.next())
					TotalOpenBalance = balanceRs.getBigDecimal(1);
			} catch (Exception e) {
				log.log(Level.SEVERE, balanceSql, e);
			} finally {
				DB.close(balanceRs, balancePstmt);
				balanceRs = null;
				balancePstmt = null;
			}

			// 2. SO_CreditUsed from completed unpaid orders
			// 'A' status: SO_CreditUsed = 已开票未付款金额(invoiceOpen精确计算) + 未开票金额
			String orderSql = "SELECT "
					// Part 1: 来自已完成订单的未付款发票金额（invoiceOpen 精确计算，付多少释放多少）
					+ "COALESCE((SELECT SUM(currencyBase(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),"
					+ "  i.C_Currency_ID,i.DateInvoiced,i.AD_Client_ID,i.AD_Org_ID)) "
					+ "  FROM C_Invoice_v i "
					+ "  WHERE i.C_BPartner_ID=bp.C_BPartner_ID AND i.IsSOTrx='Y' "
					+ "  AND i.IsPaid='N' AND i.DocStatus IN ('CO','CL') "
					+ "  AND EXISTS (SELECT 1 FROM C_Order o2 "
					+ "    WHERE o2.C_Order_ID=i.C_Order_ID AND o2.DocStatus IN ('CO','CL'))),0) "
					// Part 2: 已完成订单中尚未开票的金额（QtyOrdered - QtyInvoiced）
					+ "+ COALESCE((SELECT SUM(currencyBase("
					+ "  (ol.QtyOrdered-ol.QtyInvoiced)*ol.PriceActual,"
					+ "  o.C_Currency_ID,o.DateOrdered,o.AD_Client_ID,o.AD_Org_ID)) "
					+ "  FROM C_OrderLine ol "
					+ "  INNER JOIN C_Order o ON ol.C_Order_ID=o.C_Order_ID "
					+ "  WHERE o.C_BPartner_ID=bp.C_BPartner_ID AND o.IsSOTrx='Y' "
					+ "  AND o.DocStatus IN ('CO','CL') "
					+ "  AND ol.QtyOrdered > ol.QtyInvoiced AND ol.IsActive='Y'),0) "
					+ "FROM C_BPartner bp WHERE C_BPartner_ID=?";
			PreparedStatement orderPstmt = null;
			ResultSet orderRs = null;
			try {
				orderPstmt = DB.prepareStatement(orderSql, get_TrxName());
				orderPstmt.setInt(1, getC_BPartner_ID());
				orderRs = orderPstmt.executeQuery();
				if (orderRs.next())
					SO_CreditUsed = orderRs.getBigDecimal(1);
			} catch (Exception e) {
				log.log(Level.SEVERE, orderSql, e);
			} finally {
				DB.close(orderRs, orderPstmt);
				orderRs = null;
				orderPstmt = null;
			}

		} else {
			// Original logic: both SO_CreditUsed and TotalOpenBalance from invoices
	...
	public void setSOCreditStatus ()
	{
		...
		if (SOCREDITSTATUS_NoCreditCheck.equals(getSOCreditStatus())
			...
			|| SOCREDITSTATUS_CustomCreditCheck.equals(getSOCreditStatus())
	...
	public String getSOCreditStatus (BigDecimal additionalAmt)
	{
		...
		BigDecimal creditLimit = getSO_CreditLimit();
		//	Nothing to do
		// 原来的 creditLimit==0 豁免对 'A' 状态不适用，需要加保护
		if (SOCREDITSTATUS_NoCreditCheck.equals(getSOCreditStatus())
				|| SOCREDITSTATUS_CreditStop.equals(getSOCreditStatus())
				|| SOCREDITSTATUS_CustomCreditCheck.equals(getSOCreditStatus())  // 'A' 直接返回自身
				|| Env.ZERO.compareTo(creditLimit) == 0)
			return getSOCreditStatus();
	 ...
	 * For SOCreditStatus='A': SO_CreditLimit + TempCreditLimit (if within valid date range).
	 * @return effective credit limit
	 */
	public BigDecimal getEffectiveCreditLimit() {
		BigDecimal limit = getSO_CreditLimit();
		if (SOCREDITSTATUS_CustomCreditCheck.equals(getSOCreditStatus())) {
			BigDecimal tempLimit = getTempCreditLimit();
			if (tempLimit != null && tempLimit.signum() > 0) {
				Timestamp validFrom = getTempCreditLimitValidFrom();
				Timestamp validTo   = getTempCreditLimitValidTo();
				LocalDate today     = LocalDate.now();
				LocalDate from = validFrom != null ? validFrom.toLocalDateTime().toLocalDate() : null;
				LocalDate to   = validTo   != null ? validTo.toLocalDateTime().toLocalDate()   : null;
				boolean inRange = (from == null || !today.isBefore(from))
						&& (to   == null || !today.isAfter(to));
				if (inRange)
					limit = limit.add(tempLimit);
			}
		}
		return limit;
	}
