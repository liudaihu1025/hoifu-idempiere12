	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("DateAcct"))
			{
				...
				Object param = para[i].getParameter();
				if (param instanceof BigDecimal) {
					p_Account_ID = ((BigDecimal) param).intValue();
				} else if (param instanceof String) {
					p_Account_ID = Integer.parseInt((String) param);
				} else {
					p_Account_ID = 0;
				}
			}
...
//				p_Account_ID = ((BigDecimal) para[i].getParameter()).intValue();
	...
	private void createDetailLines()
	{
		...
		String sql3 = "UPDATE T_TrialBalance tb SET AccountName = "
				+ "(SELECT Name FROM C_ElementValue ev WHERE ev.C_ElementValue_ID=tb.Account_ID) "
				+ "WHERE tb.Account_ID IS NOT NULL AND tb.AD_PInstance_ID = " + getAD_PInstance_ID();
		no = DB.executeUpdate(sql3, get_TrxName());
		if (no > 0)
			if (log.isLoggable(Level.FINE))
				log.fine("Set AccountName #" + no);
