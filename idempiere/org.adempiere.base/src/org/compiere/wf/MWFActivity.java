...
//			msg += " - " + textMsg;
		...
		if (textMsg != null && textMsg.length() > 0) {  
		    ...
		    // 否则使用原来的逻辑（字段名=值）  
		    msg = getNode().getAttributeName() + "=" + value;  
		}  
				...
				if (!"Y".equals(value))
				{
					...
					if (!(doc.processIt (DocAction.ACTION_Reject)))
						setTextMsgBefore ("Cannot Reject - Document Status: " + doc.getDocStatus());
						 ...
						 // 修正为期望的状态 - 根据是否有历史版本设置不同状态  
				        if (doc instanceof MPPProductBOM) {  
				            MPPProductBOM productBOM = (MPPProductBOM) doc;  
				              
				            // 检查是否有历史版本  
				            if (hasHistoryVersions(productBOM)) {  
				                productBOM.setBOMStatus("InChange");  
				            } else {  
				                productBOM.setBOMStatus("InProgress");  
				            }  
					...
					if (isInvoker())
					{
						...
						else	//	Approve
						{
							if (!(doc.processIt (DocAction.ACTION_Approve)))
							{
								...
								// 添加BOM状态设置
								if (doc instanceof MPPProductBOM) {
									((MPPProductBOM) doc).setBOMStatus("Released");
									doc.saveEx();
								}
							}
						}
...
//							+ (getTextMsg() != null ? getTextMsg() : ""));
					...
					//不在消息添加单据描述字段的值
					note.setTextMsg((doc.getProcessMsg() != null ? doc.getProcessMsg() + "\n" : "")  
					        + (getTextMsg() != null ? getTextMsg() : ""));
	...
	private boolean hasHistoryVersions(MPPProductBOM productBOM) {
		String sql = "SELECT COUNT(*) FROM PP_Product_BOM WHERE M_Product_ID = ? "
				+ "AND BOMType = 'Z' AND IsActive = 'N' AND PP_Product_BOM_ID != ?";
		int count = DB.getSQLValue(productBOM.get_TrxName(), sql, productBOM.getM_Product_ID(),
				productBOM.getPP_Product_BOM_ID());
		return count > 0;
	}
