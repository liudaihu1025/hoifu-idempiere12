 ...
 */
public class MWorkflow extends X_AD_Workflow implements ImmutablePOSupport, DocAction
{
	/**
	 * generated serial id 
	 */
	...
	public MWorkflow(Properties ctx, int AD_Workflow_ID, String trxName) {
		...
		if (AD_Workflow_ID == 0) {
			...
			setPublishStatus(PUBLISHSTATUS_UnderRevision);
			setProcessed(false);
			setProcessing(false);
			setIsApproved(false);
		}
	...
	public static KeyNamePair[] getWorkflowKeyNamePairs(boolean withEmptyElement) {
		...
		if (isBaseLanguage)
			sql = MRole.getDefault().addAccessSQL(
					"SELECT AD_Workflow_ID, Name FROM AD_Workflow WHERE IsActive='Y'AND WorkflowType!='M' ORDER BY 2",
			...
			sql = MRole.getDefault().addAccessSQL(
					"SELECT AD_Workflow.AD_Workflow_ID, AD_Workflow_Trl.Name FROM AD_Workflow INNER JOIN AD_Workflow_Trl ON (AD_Workflow.AD_Workflow_ID=AD_Workflow_Trl.AD_Workflow_ID) "
							+ " WHERE AD_Workflow.IsActive='Y'AND AD_Workflow.WorkflowType!='M' AND AD_Workflow_Trl.AD_Language='"
							+ Env.getAD_Language(Env.getCtx()) + "' ORDER BY 2",
					"AD_Workflow", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO); // all
	...
	public boolean approveIt() {
		...
		// 审批通过时设置发布状态
		setPublishStatus(PUBLISHSTATUS_Released);
	...
	public String getDocumentInfo() {
		return "Workflow[" + getAD_Workflow_ID() + "-" + getValue() + "]";
	}
	 ...
	 * 验证工作流结构
	 */
	private boolean validateWorkflowStructure() {
		if (getAD_WF_Node_ID() == 0) {
			m_processMsg = "没有起始节点";
			return false;
		}
		...
		// 验证节点
		MWFNode[] nodes = getNodesInOrder(getAD_Client_ID());
		if (nodes.length == 0) {
			m_processMsg = "没有工作流节点";
			return false;
		}
