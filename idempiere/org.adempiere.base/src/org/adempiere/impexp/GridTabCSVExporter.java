...
//					   headArray.add(detail.getTableName()+">"+specialHeader);
					   headArray.add(getTranslatedTableName(detail.getTableName()) + ">" + specialHeader);
					   procArray.add(null);
				   }
...
//					    value = MRefList.getListName(Env.getCtx(),REFERENCE_PAYMENTRULE, gridTab.getValue(idxrow, header[idxfld]).toString()); 	
						Object paymentValue = gridTab.getValue(idxrow, column.getColumnName());
						if (paymentValue != null) {
							value = MRefList.getListName(Env.getCtx(), REFERENCE_PAYMENTRULE, paymentValue.toString());
						}
					}else{	
...
//				    String headName = headArray.get(headArray.indexOf(childTab.getTableName()+">"+resolveColumnName(tableDetail,column))); 
					...
					String chineseName = getTranslatedTableName(childTab.getTableName()) + ">"
							+ resolveColumnName(tableDetail, column);
...
//						row.put(childTab.getTableName()+">"+specialHeader,sValue);
						row.put(getTranslatedTableName(childTab.getTableName()) + ">" + specialHeader, sValue);
					}	
				}
		    }
	...
	private String resolveColumnName(MTable table, MColumn column) {
		// 获取主字段名的中文翻译
		String mainFieldName = getElementTranslation(column.getColumnName());
		StringBuilder name = new StringBuilder(mainFieldName);

		if (DisplayType.isLookup(column.getAD_Reference_ID()) && !DisplayType.isMultiID(column.getAD_Reference_ID())) {
			// 处理外键字段的 [] 标识符，保持内部为英文
			String foreignTable = column.getReferenceTableName();
			if ("AD_EntityType".equals(foreignTable)
					&& I_AD_EntityType.COLUMNNAME_AD_EntityType_ID.equals(column.getColumnName())) {
	...
	// ldh新增方法：从翻译表获取表名翻译
	private String getTranslatedTableName(String tableName) {  
	    String adLanguage = Env.getAD_Language(Env.getCtx());  
	    if (Env.isBaseLanguage(adLanguage, "AD_Table")) {  
	        return tableName; // 基础语言不需要翻译  
	    }  
	      
	    String sql = "SELECT t.Name FROM AD_Table_Trl t " +  
	                 "JOIN AD_Table e ON t.AD_Table_ID = e.AD_Table_ID " +  
	                 "WHERE e.TableName = ? AND t.AD_Language = ?";  
	      
	    String translatedName = DB.getSQLValueString(null, sql, tableName, adLanguage);  
	    return !Util.isEmpty(translatedName) ? translatedName : tableName;  
	}
	...
	private String getElementTranslation(String columnName) {
		...
		String sql = "SELECT Name FROM AD_Element_Trl WHERE AD_Element_ID = "
				+ "(SELECT AD_Element_ID FROM AD_Element WHERE UPPER(ColumnName) = ?) " + "AND AD_Language = ?";
        String transColumnName = DB.getSQLValueString(null, sql, columnName.toUpperCase(), adLanguage);

		return !Util.isEmpty(transColumnName) ? transColumnName : columnName;
	}
			...
			for (String fieldIdStr : fieldIds) 
			{
				...
				for (GridField gridField : tmpFields) 
				{
					if (gridField.getAD_Field_ID() == AD_Field_ID) {
						...
						// 新增：过滤明细表中的主表关联字段  
					    if (gridTab.getTabLevel() > 0 && gridField.isParentColumn()) {  
					        continue; // 跳过明细表中指向主表的外键字段  
					    }
		...
		else 
		{
			...
			for (GridField field:tmpFields)
			{
				...
				if (   field.isVirtualColumn()
						...
						continue;
					// 新增：过滤明细表中的主表关联字段
					if (gridTab.getTabLevel() > 0 && field.isParentColumn()) {
						continue; // 跳过明细表中指向主表的外键字段
					}
