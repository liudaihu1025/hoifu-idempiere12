...
//				}
		...
		// 自动校验 BOM 结构（BOM_AUTO_VERIFY=Y 时触发）
		if (MSysConfig.getBooleanValue(MSysConfig.BOM_AUTO_VERIFY, false, getAD_Client_ID())) {
			MProduct productToVerify = new MProduct(getCtx(), bom.getM_Product_ID(), get_TrxName());
			List<String> errors = new ArrayList<>();
			boolean valid = MPPProductBOM.verifyProduct(getCtx(), productToVerify, get_TrxName(), errors);
			if (!valid) {
				throw new AdempiereException("BOM 结构校验失败：\n" + String.join("\n", errors));
			}
		}
		return true;
	}
	 ...
	 * @param Keymat Keymat
	 */
	public void setKeymat(boolean Keymat) {
		set_Value(COLUMNNAME_Keymat, Boolean.valueOf(Keymat));
	}

	/** Get Keymat. */
	public boolean isKeymat() {
