...
public class InventoryValueReport extends SvrProcess {
	...
	private void insertDataFromTransaction() {
		...
		String sql = "WITH "
				...
				+ "    t.AD_Org_ID, " + "    t.M_Locator_ID, " + "    t.currentqtyonhand, "
					+ "    COALESCE(get_hist_cost_price(t.AD_Client_ID, t.AD_Org_ID, t.M_Product_ID, t.Created), 0) AS hist_cost_price, "
				+ "    ROW_NUMBER() OVER ( "
				...
				+ "    SUM(currentqtyonhand * hist_cost_price) AS amt, " + "    CASE WHEN SUM(currentqtyonhand) > 0 "
				+ "      THEN SUM(currentqtyonhand * hist_cost_price) / SUM(currentqtyonhand) " + "      ELSE 0 "
				+ "    END AS price " + "  FROM beginning_base " + "  WHERE rn = 1 "
				+ "  GROUP BY M_Product_ID, AD_Client_ID, AD_Org_ID " + "), "

				// ========= 本期入库数据：按物料+组织汇总 =========
				+ "inbound AS ( " + "  SELECT " + "    M_Product_ID, " + "    AD_Client_ID, " + "    AD_Org_ID, "
				+ "    SUM(qty_pos) AS qty, "
				+ "    CASE WHEN SUM(qty_pos) > 0 "
				+ "      THEN SUM(qty_pos * hist_cost_price) / SUM(qty_pos) " + "      ELSE 0 "
				+ "    END AS price, "
				+ "    SUM(qty_pos * hist_cost_price) AS amt "
				+ "  FROM ( "
				+ "    SELECT t.M_Product_ID, t.AD_Client_ID, t.AD_Org_ID, "
				+ "      CASE WHEN t.MovementQty > 0 THEN t.MovementQty ELSE 0 END AS qty_pos, "
				+ "      COALESCE(get_hist_cost_price(t.AD_Client_ID, t.AD_Org_ID, t.M_Product_ID, t.Created), 0) AS hist_cost_price "
				...
				// ========= 本期出库数据：按物料+组织汇总 =========
				+ "outbound AS ( " + "  SELECT " + "    M_Product_ID, " + "    AD_Client_ID, " + "    AD_Org_ID, "
				+ "    SUM(qty_neg) AS qty, "
				+ "    CASE WHEN SUM(qty_neg) > 0 "
				+ "      THEN SUM(qty_neg * hist_cost_price) / SUM(qty_neg) " + "      ELSE 0 "
				+ "    END AS price, "
				+ "    SUM(qty_neg * hist_cost_price) AS amt "
				+ "  FROM ( "
				+ "    SELECT t.M_Product_ID, t.AD_Client_ID, t.AD_Org_ID, "
				+ "      CASE WHEN t.MovementQty < 0 THEN ABS(t.MovementQty) ELSE 0 END AS qty_neg, "
				+ "      COALESCE(get_hist_cost_price(t.AD_Client_ID, t.AD_Org_ID, t.M_Product_ID, t.Created), 0) AS hist_cost_price "
				+ "    FROM M_Transaction t " + "    INNER JOIN M_Product p ON (t.M_Product_ID = p.M_Product_ID) "
				+ "    WHERE t.MovementDate >= ? AND t.MovementDate <= ? AND " + m_parameterWhere.toString()
				+ "  ) outbound_base "
