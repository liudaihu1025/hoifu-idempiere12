...
public class BillReceiptMaturityProcess extends SvrProcess {
	// 参数
	...
	private BigDecimal p_CollectionFee = null;// 收款费用
	...
	protected void prepare() {
		...
		for (int i = 0; i < para.length; i++) {
			...
			else if (name.equals("CollectionFee"))
				p_CollectionFee = (BigDecimal) para[i].getParameter();// 收款费用
	...
	protected String doIt() throws Exception {
		...
		StringBuilder resultMsg = new StringBuilder("成功处理 ").append(processedCount).append(" 张票据");
	...
	private void processBillReceipt(MBillPool billPool) throws Exception {
		// 设置状态
		if (p_IsDishonored) {
			billPool.setBusinessStatus("V"); // 对方拒付
		} else {
		...
		billPool.set_ValueNoCheck("SettleFeeAmt", p_CollectionFee); // 结算费用
		if (p_CollectionFee != null) {
			billPool.set_ValueNoCheck("SettleAmt", billPool.getBillAmt().add(p_CollectionFee)); // 结算金额
		}
	...
	private void createBillTransaction(MBillPool billPool) throws Exception {
		...
		transaction.setTransactionType("R"); // 收款
		...
		// 设置票据相关信息
		transaction.set_ValueOfColumn("IsRecourse", billPool.isRecourse() ? "Y" : "N");
	...
	private void createReceipt(MBillPool billPool) throws Exception {
		...
		// 设置收款事项
		payment.setR_Info("票据到期收款 - " + billPool.getDocumentNo());
