...
public class WMSReceiptRequestLine extends SvrProcess {
	...
	protected String doIt() throws Exception {  
	    ...
	    // 用于记录目标单据的完成状态，判断是否需要在返回信息里追加提示  
	    boolean notCompleted = false;  
	  
	    if (MInOut.Table_Name.equals(tableName))  
	    {  
	        MInOut inOut = InOutRequisitionToInOutService.createFromRequisition(p_DocumentNo, p_Line, p_Qty, getCtx(), get_TrxName());  
	        if (inOut != null && !DocAction.STATUS_Completed.equals(inOut.getDocStatus()))  
	            notCompleted = true;  
	    }  
	    if (MInventory.Table_Name.equals(tableName))  
	    {  
	        MInventory inventory = InOutRequisitionToInventoryService.createFromRequisition(p_DocumentNo, p_Line, p_Qty, getCtx(), get_TrxName());  
	        if (inventory != null && !DocAction.STATUS_Completed.equals(inventory.getDocStatus()))  
	            notCompleted = true;  
	    }  
	    if (X_PP_Cost_Collector.Table_Name.equals(tableName))  
	    {  
	        GenericPO ppCostCollector = InOutRequisitionToPPMaterialOrderService.createFromRequisition(p_DocumentNo, p_Line, p_Qty, getCtx(), get_TrxName());  
	        if (ppCostCollector != null && !DocAction.STATUS_Completed.equals(ppCostCollector.getDocStatus()))  
	            notCompleted = true;  
	    }  
	    ...
	    return notCompleted ? "请求成功（该单据未完成）" : "请求成功";  
	}
}