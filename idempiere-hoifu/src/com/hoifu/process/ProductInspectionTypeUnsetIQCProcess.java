...
/**
 * 批量取消物料检验类型：勾选多个 M_Product，
 * 校验物料是否有效（IsActive），若检验类型（InspectionType）为 来料检验(IQC)，
 * 则修改为 无(NONE)。
 *
 * 支持两种获取物料ID的方式（合并去重）：
...
public class ProductInspectionTypeUnsetIQCProcess extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		for (Integer productId : selectedProductIds) {
			...
			// 仅当当前为 来料检验(IQC) 时才能改为 无(NONE)
			if (INSPECTIONTYPE_IQC.equals(inspectionType)) {
				product.set_ValueOfColumn("InspectionType", INSPECTIONTYPE_NONE);
				if (product.save()) {
					updatedCount++;
				} else {
				...
				if (log.isLoggable(Level.FINE))
					log.fine("物料[" + product.getName() + "]检验类型非[IQC]，当前值=" + inspectionType + "，跳过");
			}
		}

		return "已更新 " + updatedCount + " 条，跳过停用 " + skippedInactive
				+ " 条，跳过非[IQC]检验类型 " + skippedNotIQC + " 条";
	}
