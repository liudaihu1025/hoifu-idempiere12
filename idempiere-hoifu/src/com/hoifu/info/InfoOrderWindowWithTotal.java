...
/**
 * Info window for Purchase Order (C_Order) with GrandTotal column total (UI
 * only).
 */
public class InfoOrderWindowWithTotal extends InfoWindow {
	private static final long serialVersionUID = 1L;

	// 需要求和的列名（与 AD_InfoColumn 的显示标题一致）
	private static final String[] TOTAL_COLUMNS = { "总额" };

	public InfoOrderWindowWithTotal(int WindowNo, String tableName, String keyColumn, String queryValue,
		...
		Executions.schedule(table.getDesktop(), new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				try {
					...
					if (totalItem != null) {
						// 设置整行背景色
						ZkCssHelper.appendStyle(totalItem, "background-color: #E6F3FF !important;");
						...
						if (leftmostTotalColumn > 0) {
							Listcell sigmaCell = totalItem.getChildren().size() > leftmostTotalColumn - 1
									...
									: null;
							if (sigmaCell != null) {
								ZkCssHelper.appendStyle(sigmaCell, "font-weight: bold");
							}
						}
						...
						for (String columnName : TOTAL_COLUMNS) {
							...
							if (colIndex != null) {
								Listcell cell = totalItem.getChildren().size() > colIndex
										...
										: null;
								if (cell != null) {
									ZkCssHelper.appendStyle(cell, "font-weight: bold");
								}
							}
						}
                    }  
