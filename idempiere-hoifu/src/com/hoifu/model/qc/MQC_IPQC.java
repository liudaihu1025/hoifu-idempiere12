...
public class MQC_IPQC extends X_QC_IPQC implements DocAction, IQCDocument {
	private static final long serialVersionUID = 1L;

	private String m_processMsg = null;
	private boolean m_justPrepared = false;

	public MQC_IPQC(Properties ctx, int QC_IPQC_ID, String trxName) {
		super(ctx, QC_IPQC_ID, trxName);
	}

	public MQC_IPQC(Properties ctx, ResultSet rs, String trxName) {
	...
	public static int getDocTypeID(Properties ctx, int AD_Client_ID, int AD_Org_ID, String trxName) {
		String sql = "SELECT C_DocType_ID FROM C_DocType "
				+ "WHERE AD_Client_ID=? AND AD_Org_ID IN (0,?) AND DocBaseType='PQC' "
				+ "AND IsActive='Y' ORDER BY AD_Org_ID DESC, IsDefault DESC";
		return DB.getSQLValueEx(trxName, sql, AD_Client_ID, AD_Org_ID);
	}

	public List<MQC_IPQCLine> getLines() {
		return new Query(getCtx(), MQC_IPQCLine.Table_Name, "QC_IPQC_ID=?", get_TrxName())
				.setParameters(getQC_IPQC_ID()).list();
	}
	...
	public String prepareIt() {
		...
		List<MQC_IPQCLine> lines = getLines();
		if (lines.isEmpty()) {
			m_processMsg = "过程检验单无明细行，无法完成";
			return DocAction.STATUS_Invalid;
		}
//		for (MQC_IPQCLine line : lines) {
...
//		WriteBackHandler.writeBackIPQCResult(this);
