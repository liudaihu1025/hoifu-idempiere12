...
public class RollupBillOfMaterial extends SvrProcess
{
	...
	private void resetCostsLLForLLC0()
	{
		...
		StringBuffer costWhereClause = new StringBuffer();
		...
		if (p_ConstingMethod != null)
		{
			costWhereClause.append(" AND mc.").append(MCost.COLUMNNAME_CostingMethod).append("=?");
			params.add(p_ConstingMethod);
		}
		//
		final String sql = "UPDATE M_Cost c SET "+MCost.COLUMNNAME_CurrentCostPriceLL+"=0 FROM M_CostElement mc"
		+" WHERE c.M_CostElement_ID = mc.M_CostElement_ID AND EXISTS (SELECT 1 FROM M_Product p WHERE p.M_Product_ID=c.M_Product_ID"
						+" AND "+productWhereClause+")" + costWhereClause;
