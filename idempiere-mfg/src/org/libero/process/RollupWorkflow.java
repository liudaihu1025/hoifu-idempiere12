	...
	public void rollup(MProduct product, MWorkflow workflow)
	{
		...
		for (MCostElement element : MCostElement.getByCostingMethod(getCtx(), p_ConstingMethod))
		{
			...
			for (MCost cost : costs)
			{
				...
				// Create a temporary cost collector for cost calculation
				MPPCostCollector tempcc = new MPPCostCollector(getCtx(), 0, get_TrxName());
				tempcc.setAD_Org_ID(p_AD_Org_ID);
				tempcc.setPP_Order_ID(0);

				for (MWFNode node : nodes)
				{
					final CostEngine costEngine = CostEngineFactory.getCostEngine(node.getAD_Client_ID());
					final BigDecimal rate = costEngine.getResourceActualCostRate(tempcc, node.getS_Resource_ID(), d, get_TrxName());
