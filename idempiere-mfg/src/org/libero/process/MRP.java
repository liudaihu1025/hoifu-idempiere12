...
*/			
		...
		resultMsg.append("</br>" + Msg.getMsg(getCtx(), "MRP-ProcessStarted") + " " + startTimeStr);    
		resultMsg.append("</br>" + Msg.getMsg(getCtx(), "MRP-ProcessFinished") + " " + endTimeStr +   
		                 ", " + Msg.getMsg(getCtx(), "MRP-ProcessDuration") + " " + elapsedSeconds + " " + Msg.getMsg(getCtx(), "Second"));
		
		 // Add selected parameters display  
	    resultMsg.append("<br>参数：" + getOrgName() +   
	                    " - " + getPlantName() +   
	                    " - " + getWarehouseName());
		
		 //add resultMsg format by PShepetko       
	    resultMsg.append("<br>" + Msg.getMsg(getCtx(), "MRP-ResultCreated") +   
	        " " + Msg.getMsg(getCtx(), "MRP-ResultMO") + "=" + count_MO +   
	        "个, " + Msg.getMsg(getCtx(), "MRP-ResultDO") + "=" + count_DO +  
	        "个, " + Msg.getMsg(getCtx(), "MRP-ResultMR") + "=" + count_MR +  
	        "个, " + Msg.getMsg(getCtx(), "MRP-ResultNotice") + "=" + count_Msg +"个" 
	       );  
...
//	}
								...
								MProduct product, Timestamp DemandDateStartSchedule) throws SQLException
	{
		...
		int precision = MUOM.getPrecision(getCtx(), product.getC_UOM_ID());
		final BigDecimal yield = BigDecimal.valueOf(m_product_planning.getYield());
		if (yield.signum() != 0)
		{
			QtyGrossReqs = QtyGrossReqs.multiply(Env.ONEHUNDRED).divide(yield, precision, RoundingMode.UP);
		}
	...
	{		
		...
		{	
			...
			MShipper shipper = new MShipper(getCtx(), network_line.getM_Shipper_ID(), get_TrxName());      
		          
		        DocumentInfo docInfo = new DocumentInfo(  
		            order.get_ID(),                           // 配送订单ID  
		            order.getDocumentNo(),                    // 配送订单号  
		            productInfo.getValue(),                   // 物料编码  
		            productInfo.getName(),                    // 物料名称  
		            productInfo.get_ID(),                     // 物料ID（用于链接）  
		            order.getDateOrdered(),                   // 订单日期  
		            order.getDatePromised(),                  // 承诺交期  
		            QtyOrdered,                               // 数量  
		            MUOM.get(getCtx(), productInfo.getC_UOM_ID()).getUOMSymbol(), // 单位  
		            shipper.getName()                         // 承运商/运输方式  
		        );  
		          
		        // 添加到配送订单信息列表  
		        createdDOInfos.add(docInfo);  
...
//		order.setDateFinishSchedule(DemandDateStartSchedule);
		...
		int workingTime = 0;
	    int transferTime = 0;  
	    int deliveryTimePromised = 0;  
	    
	    // 获取工作时间（转换为分钟）  
	    if (m_product_planning != null && m_product_planning.getWorkingTime() != null) {  
	        workingTime = m_product_planning.getWorkingTime().intValueExact() * 24 * 60;  
	    } 
	    
	    // 获取转序时间（转换为分钟）
	    if (m_product_planning != null && m_product_planning.getTransfertTime() != null) {  
	        transferTime = m_product_planning.getTransfertTime().intValueExact() * 24 * 60;  
	    }  
	    
	    // 获取采购承诺交期
	    if (m_product_planning != null && m_product_planning.getDeliveryTime_Promised() != null) {  
	        deliveryTimePromised = m_product_planning.getDeliveryTime_Promised().intValueExact() * 24 * 60;  
	    }  
	      
	    int totalDuration = workingTime + transferTime + deliveryTimePromised;  
	      
	    // 使用分钟而不是天进行计算  
	    order.setDateStartSchedule(TimeUtil.addMinutess(DemandDateStartSchedule, 0 - totalDuration));  
	    order.setDateFinishSchedule(DemandDateStartSchedule);  
		...
		try {
			MPPMRP mrp = new Query(getCtx(), MPPMRP.Table_Name, "PP_MRP_ID=?", get_TrxName())  
				    ...
				    .first();  
				if (mrp != null && mrp.getC_OrderLine_ID() > 0) {  
				    order.setC_OrderLine_ID(mrp.getC_OrderLine_ID());  
				} 
		}
		catch (Exception e) {
			log.severe("创建工单设置C_OrderLine_ID错误： - " + e.getMessage());
		}
		order.saveEx();
		...
		// 收集生产工单详细信息    
		MProduct productInfo = MProduct.get(getCtx(), order.getM_Product_ID());      
		log.info("Order ID: " + order.get_ID());  
		log.info("DocumentNo: " + order.getDocumentNo());  
		log.info("DatePromised: " + order.getDatePromised());  
		log.info("DateOrdered: " + order.getDateOrdered());  
		log.info("S_Resource_ID: " + order.getS_Resource_ID());  
		  
		// Debug: Check if the order was loaded from database correctly  
		if (order.get_ID() <= 0) {  
		    log.warning("Order ID is invalid: " + order.get_ID());  
		...
		DocumentInfo docInfo = new
				 DocumentInfo(        
				    ...
				    order.getQtyOrdered(),                       
				    MUOM.get(getCtx(), productInfo.getC_UOM_ID()).getUOMSymbol(),     
				    order.getS_Resource_ID() > 0 ? MResource.get(getCtx(), order.getS_Resource_ID()).getName() : ""
				,  
				    true                                        // Added boolean parameter for manufacturing order  
				);    
		
		createdMOInfos.add(docInfo);  
		count_MO += 1;
	}
...
//		 + ":" + DemandDateStartSchedule;
	...
	private void displayCreatedDocuments() {  
	    ...
	    if (!createdMOInfos.isEmpty()) {  
	        addBufferLog(0, null, null, "生产工单", MPPOrder.Table_ID, 0);  
	          
	        // 总是显示最多10条记录  
	        int displayCount = Math.min(10, createdMOInfos.size());  
	        List<DocumentInfo> displayList = createdMOInfos.subList(0, displayCount);  
	        String[][] moTable = createManufacturingOrderTable(displayList);  
	        String htmlTable = createHTMLTable(moTable);  
	        addBufferLog(0, null, null, htmlTable, MPPOrder.Table_ID, 0);  
	          
	        // 如果超过10条，显示提示信息  
	        if (createdMOInfos.size() > 10) {  
	            addBufferLog(0, null, null, "记录已超过10条，全部记录请在生产工单窗口中查看", MPPOrder.Table_ID, 0);  
	        }  
	    }  
	        
	    // 配送订单表格  
	    if (!createdDOInfos.isEmpty()) {  
	        addBufferLog(0, null, null, "配送订单", MDDOrder.Table_ID, 0);  
	          
	        // 总是显示最多10条记录  
	        int displayCount = Math.min(10, createdDOInfos.size());  
	        List<DocumentInfo> displayList = createdDOInfos.subList(0, displayCount);  
	        String[][] doTable = createDistributionOrderTable(displayList);  
	        String htmlTable = createHTMLTable(doTable);  
	        addBufferLog(0, null, null, htmlTable, MDDOrder.Table_ID, 0);  
	          
	        // 如果超过10条，显示提示信息  
	        if (createdDOInfos.size() > 10) {  
	            addBufferLog(0, null, null, "记录已超过10条，全部记录请在配送订单窗口中查看", MDDOrder.Table_ID, 0);  
	        }  
	    }  
	}
	  
	private String[][] createRequisitionTable(List<DocumentInfo> reqInfos) {  
	    ...
	    for (int i = 0; i < reqInfos.size(); i++) {  
	        DocumentInfo docInfo = reqInfos.get(i);  
	        table[i + 1][0] = docInfo.documentNo; // 申购单号  
	        table[i + 1][1] = createProductLink(docInfo.productValue, docInfo.productName); // 物料  
	        table[i + 1][2] = docInfo.requisitionDate != null ? formatDate(docInfo.requisitionDate): ""; // 申购日期  
	        table[i + 1][3] = docInfo.requiredDate != null ? formatDate(docInfo.requiredDate)  : ""; // 需求日期  
	        table[i + 1][4] = docInfo.quantity != null ? docInfo.quantity.toString() : ""; // 数量  
	        table[i + 1][5] = docInfo.uom; // 单位  
	        table[i + 1][6] = docInfo.supplier; // 供应商  
	    }  
	      
	    return table;  
	}  
	  
	private String[][] createManufacturingOrderTable(List<DocumentInfo> moInfos) {  
	    String[][] table = new String[moInfos.size() + 1][7]; // +1 for header  
	      
	    
	    table[0] = new String[]{"工程单号", "物料", "工单日期", "承诺交期", "数量", "单位", "资源"};  
	      
	    // 数据行  
	    for (int i = 0; i < moInfos.size(); i++) {  
	        DocumentInfo docInfo = moInfos.get(i);  
	        table[i + 1][0] = docInfo.documentNo; // 申购单号 -> 工程单号  
	        table[i + 1][1] = createProductLink(docInfo.productValue, docInfo.productName); // 物料  
	        table[i + 1][2] = docInfo.orderDate != null ? formatDate(docInfo.orderDate) : ""; // 申购日期 -> 工单日期  
	        table[i + 1][3] = docInfo.datePromised != null ? formatDate(docInfo.datePromised) : ""; // 需求日期 -> 承诺交期  
	        table[i + 1][4] = docInfo.quantity != null ? docInfo.quantity.toString() : ""; // 数量  
	        table[i + 1][5] = docInfo.uom; // 单位  
	        table[i + 1][6] = docInfo.resource; // 供应商 -> 资源  
	    }  
	      
	    return table;  
	}  
	  
	private String[][] createDistributionOrderTable(List<DocumentInfo> doInfos) {  
	    String[][] table = new String[doInfos.size() + 1][7]; // +1 for header  
	      
	    
	    table[0] = new String[]{"配送单号", "物料", "配送日期", "承诺日期", "数量", "单位", "仓库"};  
	      
	    // 数据行  
	    for (int i = 0; i < doInfos.size(); i++) {  
	        DocumentInfo docInfo = doInfos.get(i);  
	        table[i + 1][0] = docInfo.documentNo; // 申购单号 -> 配送单号  
	        table[i + 1][1] = createProductLink(docInfo.productValue, docInfo.productName); // 物料  
	        table[i + 1][2] = docInfo.dateOrdered != null ? formatDate(docInfo.dateOrdered) : ""; // 申购日期 -> 配送日期  
	        table[i + 1][3] = docInfo.datePromised != null ? formatDate(docInfo.datePromised) : ""; // 需求日期 -> 承诺日期  
	        table[i + 1][4] = docInfo.quantity != null ? docInfo.quantity.toString() : ""; // 数量  
	        table[i + 1][5] = docInfo.uom; // 单位  
	        table[i + 1][6] = docInfo.warehouse; // 供应商 -> 仓库  
	    }  
	      
	    return table;  
	}
	  
	private String createProductLink(String productValue, String productName) {  
	    // 返回 productValue + productName 的拼接值，不加链接  
	    if (productValue != null && productName != null) {  
	        return productValue + " " + productName;  
