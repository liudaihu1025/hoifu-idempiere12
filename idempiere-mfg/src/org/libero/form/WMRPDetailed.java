...
public class WMRPDetailed extends MRPDetailed implements IFormController, EventListener, ListDataListener, WTableModelListener {
	...
	protected void jbInit() throws Exception
	{
		...
		//p_table.setFixedLayout(true); 	z5k1 todo 
		//center.setFlex(true); 			z5k1 todo
		...
		// 获取OK按钮并设置文本为"计算"  
		confirmPanel.getOKButton().setLabel("开始计算");
...
//			m_frame.dispose();
	 ...
	 * 计算选中的 MRP 记录(支持多选)
	 */
	private void calculateSelectedMRP() {
		try {
			// 获取所有选中行
			int[] selectedRows = p_table.getSelectedIndices();

			if (selectedRows == null || selectedRows.length == 0) {
				Dialog.warn(m_WindowNo, "请先选择要计算的 MRP 记录");
				return;
			}

			// 显示处理中提示
			statusBar.setStatusLine("正在准备 MRP 计算...", false);

			// 查找 CalculateMaterialPlan 进程
			// 方案1: 通过类名查找进程
			String processClassName = "org.libero.process.CalculateMaterialPlan";
			int AD_Process_ID = DB.getSQLValue(null, "SELECT AD_Process_ID FROM AD_Process WHERE Classname=?",
					processClassName);

			// 方案2: 如果方案1失败,尝试通过进程名称查找
//			if (AD_Process_ID <= 0) {
//				AD_Process_ID = DB.getSQLValue(null,
//						"SELECT AD_Process_ID FROM AD_Process WHERE Name LIKE '%Material Plan%' OR Name LIKE '%MRP%' ORDER BY AD_Process_ID DESC");
//			}

			if (AD_Process_ID <= 0) {
				Dialog.error(m_WindowNo, "找不到 MRP 计算进程,请联系系统管理员配置");
				return;
			}

			// 检查进程访问权限
			boolean hasAccess = MRole.getDefault().getProcessAccess(AD_Process_ID);
			if (!hasAccess) {
				Dialog.error(m_WindowNo, "当前角色没有权限执行此进程,请联系系统管理员");
				return;
			}

			// 创建进程信息
			ProcessInfo pi = new ProcessInfo("计算物料计划", AD_Process_ID);
			pi.setAD_Client_ID(Env.getAD_Client_ID(getCtx()));
			pi.setAD_User_ID(Env.getAD_User_ID(getCtx()));
			pi.setTable_ID(MPPMRP.Table_ID);
			pi.setClassName(processClassName);
			...
			for (int row : selectedRows) {
				...
				if (data instanceof IDColumn) {
					mrpId = ((IDColumn) data).getRecord_ID();
				} else if (data instanceof Integer) {
					mrpId = (Integer) data;
				}

				if (mrpId != null && mrpId > 0) {
					selectedIds.add(mrpId);
				}
			}

			if (selectedIds.isEmpty()) {
				Dialog.error(m_WindowNo, "没有有效的 MRP 记录被选中");
				return;
			}

			// 设置选中的记录ID到ProcessInfo
			pi.setRecord_IDs(selectedIds);

			// 更新状态栏
			final int selectionCount = selectedIds.size();
			statusBar.setStatusLine("正在计算 " + selectionCount + " 条 MRP 记录...", false);

			// 使用 WProcessCtl 执行进程,并提供回调处理结果
			WProcessCtl.process(m_WindowNo, pi, null, new EventListener<Event>() {
				@Override
				public void onEvent(Event event) throws Exception {
					if (event.getTarget() instanceof ProcessModalDialog) {
						ProcessModalDialog dialog = (ProcessModalDialog) event.getTarget();

						if (ProcessModalDialog.ON_WINDOW_CLOSE.equals(event.getName())) {
							if (dialog.isCancel()) {
								statusBar.setStatusLine("MRP 计算已取消", false);
							} else {
								ProcessInfo result = dialog.getProcessInfo();
								if (result.isError()) {
									Dialog.error(m_WindowNo, "MRP 计算失败", result.getSummary());
									statusBar.setStatusLine("MRP 计算失败", true);
								} else {
									Dialog.info(m_WindowNo, "MRP 计算完成", "成功计算 " + selectionCount + " 条 MRP 记录\n" + result.getSummary());
									statusBar.setStatusLine("MRP 计算完成", false);
			...
			log.log(Level.SEVERE, "MRP 计算失败", e);
			Dialog.error(m_WindowNo, "MRP 计算失败: " + e.getMessage());
			statusBar.setStatusLine("MRP 计算失败", true);
		}
	}
