...
public class Callout_HF_QuotationPrice implements IColumnCallout {  
	...
	private String onProduct(Properties ctx, GridTab mTab, Object value) {  
		...
		// 报价纸质（CartonMaterial_ID）  
		Object cartonMaterialIdObj = product.get_Value("CartonMaterial_ID");  
		mTab.setValue("CartonMaterial_ID", cartonMaterialIdObj);  
  
		if (cartonMaterialIdObj instanceof Integer) {  
			int cartonMaterialId = (Integer) cartonMaterialIdObj;  
			if (cartonMaterialId > 0) {  
				// 纸板厚度来自纸质产品  
				MProduct paperProduct = MProduct.get(ctx, cartonMaterialId);  
				...
				MProductPrice pp = new Query(ctx, MProductPrice.Table_Name,  
						...
						.setParameters(cartonMaterialId)  
	...
	/** 含税单价 = 纸板成本 ÷ 报价系数 × 100（结果保留4位小数） */  
	private void recalcPriceWithTax(GridTab mTab) {  
		BigDecimal CalcRatio = getBD(mTab, "CalcRatio");  
		if (CalcRatio.compareTo(BigDecimal.ZERO) != 0) {  
			BigDecimal result = getBD(mTab, "BoardCost")  
					.divide(CalcRatio, 4, RoundingMode.HALF_UP);  
			mTab.setValue("PriceWithTax", result);  
		} else {  
			mTab.setValue("PriceWithTax", BigDecimal.ZERO);  
		}  
	}  
  
	/** 报价材质价 = 纸质价 × 排版系数（结果保留4位小数） */  
	private void recalcQuoteMaterialPrice(GridTab mTab) {  
		...
		mTab.setValue("QuoteMaterialPrice",  
				priceStd.multiply(layoutFactor).setScale(4, RoundingMode.HALF_UP));  
	}  
  
	/** 纸板成本 = 报价材质价 × 纸板面积（结果保留4位小数） */  
	private void recalcBoardCost(GridTab mTab) {  
		...
		mTab.setValue("BoardCost",  
				quoteMaterialPrice.multiply(cardArea).setScale(4, RoundingMode.HALF_UP));  
	}  
	...
	private void recalcSalesFee(GridTab mTab) {  
		...
		mTab.setValue("SalesFee", price.multiply(rate).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
  
	private void recalcRebate(GridTab mTab) {  
		mTab.setValue("Rebate",  
				getBD(mTab, "PriceWithTax").multiply(getBD(mTab, "RebateRate")).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
  
	private void recalcSpareFee(GridTab mTab) {  
		mTab.setValue("SpareFee",  
				getBD(mTab, "PriceWithTax").multiply(getBD(mTab, "SpareRate")).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
  
	/** 损耗费 = 纸板成本 × 损耗率（结果保留4位小数） */  
	private void recalcWasteFee(GridTab mTab) {  
		mTab.setValue("WasteFee",  
				getBD(mTab, "BoardCost").multiply(getBD(mTab, "WasteRate")).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
  
	private void recalcFreightAmt(GridTab mTab) {  
		mTab.setValue("FreightAmt",  
				getBD(mTab, "PriceWithTax").multiply(getBD(mTab, "FreightRate")).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
  
	private void recalcComprehensiveFee(GridTab mTab) {  
		mTab.setValue("ComprehensiveFee",  
				getBD(mTab, "PriceWithTax").multiply(getBD(mTab, "ComprehensiveRate")).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
  
	private void recalcTaxAmt(GridTab mTab) {  
		mTab.setValue("TaxAmt",  
				getBD(mTab, "PriceWithTax").multiply(getBD(mTab, "taxrate")).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
  
	private void recalcInterestAmt(GridTab mTab) {  
		mTab.setValue("InterestAmt",  
				getBD(mTab, "PriceWithTax").multiply(getBD(mTab, "InterestRate")).divide(BD100, 4, RoundingMode.HALF_UP));  
	}  
	...
	/**  
	 * 纸板成本+加工费 = 纸板成本 + 分纸费 + 印刷费 + 开槽费 + 啤机费 + 粘合费 + 钉箱费 + 清废费 + 打包费 + 损耗费  
	 * 其他费用 = 业务费 + 返点 + 备品费 + 胶版费 + 刀模费 + 运费 + 综合费用 + 税费 + 贴息  
	 * 合计费用 = 纸板成本+加工费 + 其他费用  
	 * 利润 = 含税单价 - 合计费用  
	 * 利润率 = 利润 / 含税单价 × 100  
	 * 所有结果保留4位小数  
	...
	private void recalcSummary(GridTab mTab) {  
		// 纸板成本+加工费  
		BigDecimal boardAndProcessCost = getBD(mTab, "BoardCost")  
				.add(getBD(mTab, "PaperCuttingFee"))  
				.add(getBD(mTab, "PrintingFee"))  
				.add(getBD(mTab, "SlottingFee"))  
				.add(getBD(mTab, "DieCuttingFee"))  
				.add(getBD(mTab, "GluingFee"))  
				.add(getBD(mTab, "StitchingFee"))  
				.add(getBD(mTab, "WasteCleaningFee"))  
				.add(getBD(mTab, "PackingFee"))  
				.add(getBD(mTab, "WasteFee"))  
				.add(getBD(mTab, "OtherProcessFee"))  
				.setScale(4, RoundingMode.HALF_UP);  
		mTab.setValue("BoardAndProcessCost", boardAndProcessCost);  
  
		// 其他费用  
		BigDecimal otherFee = getBD(mTab, "SalesFee")  
				.add(getBD(mTab, "Rebate"))  
				.add(getBD(mTab, "SpareFee"))  
				.add(getBD(mTab, "PlateFee"))  
				.add(getBD(mTab, "DieFee"))  
				.add(getBD(mTab, "FreightAmt"))  
				.add(getBD(mTab, "ComprehensiveFee"))  
				.add(getBD(mTab, "TaxAmt"))  
				.add(getBD(mTab, "InterestAmt"))  
				.setScale(4, RoundingMode.HALF_UP);  
		mTab.setValue("OtherFee", otherFee);  
  
		// 合计费用  
		BigDecimal totalFee = boardAndProcessCost.add(otherFee).setScale(4, RoundingMode.HALF_UP);  
		mTab.setValue("TotalFee", totalFee);  
  
		// 利润 & 利润率  
		BigDecimal priceWithTax = getBD(mTab, "PriceWithTax");  
		BigDecimal profit = priceWithTax.subtract(totalFee).setScale(4, RoundingMode.HALF_UP);  
		mTab.setValue("Profit", profit);  
  
		if (priceWithTax.compareTo(BigDecimal.ZERO) != 0) {  
			BigDecimal profitRate = profit.divide(priceWithTax, 4, RoundingMode.HALF_UP).multiply(BD100)  
					.setScale(4, RoundingMode.HALF_UP);  
