...
public class CalloutBOM extends CalloutEngine
{
	...
	public String parent (Properties ctx, int WindowNo, GridTab mTab, GridField  mField, Object value)
	{
		...
		String lengbie = (String) mProduct.get_Value("lengbie");
		if (lengbie != null) {
			mTab.setValue("Lengbie", lengbie);
		}

		// NEW: Check L1 product category and set keymat
		String sql = "SELECT pc.Value FROM M_Product_Category pc "
				+ "INNER JOIN M_Product p ON p.M_Product_Category_ID_L1 = pc.M_Product_Category_ID "
				+ "WHERE p.M_Product_ID = ?";
		String l1CategoryValue = DB.getSQLValueString(null, sql, M_Product_ID);

		if (l1CategoryValue != null) {
			if ("ZZ".equals(l1CategoryValue) || "KZ".equals(l1CategoryValue) || "ZB".equals(l1CategoryValue)) {
				bomLine.setKeymat("Y");
			} else {
				bomLine.setKeymat("N");
			}
