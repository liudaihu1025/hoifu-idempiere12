...
//		}
	...
	public String user(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		// 判断是否为生产报工类型
		I_PP_Cost_Collector cc = GridTabWrapper.create(mTab, I_PP_Cost_Collector.class);
		String costCollectorType = cc.getCostCollectorType();

		// 只有生产报工类型才执行默认班组逻辑
		if (!MPPCostCollector.COSTCOLLECTORTYPE_ActivityControl.equals(costCollectorType)) {
			return "";
		}
		Integer adUserId = (Integer) value;
		if (adUserId == null || adUserId == 0)
			return "";

		// 查找该用户的默认班组（IsDefault='Y' 的成员记录）
		int teamId = DB.getSQLValue(null, "SELECT m.C_WorkTeam_ID FROM C_WorkTeamMember m "
				+ "WHERE m.AD_User_ID=? AND m.IsDefault='Y' AND m.IsActive='Y' " + "ORDER BY m.C_WorkTeam_ID LIMIT 1",
				adUserId);

		if (teamId > 0)
			mTab.setValue("C_WorkTeam_ID", teamId);
		else
			mTab.setValue("C_WorkTeam_ID", null);
