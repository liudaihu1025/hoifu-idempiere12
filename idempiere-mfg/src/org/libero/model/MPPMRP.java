...
//			mrp = getQuery(ol, null, null).firstOnly();
			// 添加更严格的查询条件，避免多条记录
			mrp = getQuery(ol, MPPMRP.TYPEMRP_Supply, MPPMRP.ORDERTYPE_PurchaseOrder).firstOnly();
		}
		//<--Ferry		
		if(mrp == null)
		{	
	...
	public static void M_RequisitionLine(MRequisitionLine rl)
	{
	    ...
	    if (mrp.getPP_Order_ID() > 0) {  
	        MPPOrder order = new MPPOrder(rl.getCtx(), mrp.getPP_Order_ID(), rl.get_TrxName()); 
	        rl.set_ValueOfColumn("ENo", order.getDocumentNo());
	    }  
		//<--Ferry
