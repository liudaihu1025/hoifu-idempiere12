...
public class DefaultRoutingServiceImpl implements RoutingService
{
	...
	public BigDecimal estimateWorkingTime(I_PP_Order_Node node, BigDecimal qty)
	{
		...
		// 修改为（避免 double 精度问题）
		BigDecimal duration = BigDecimal.valueOf(unitDuration).multiply(BigDecimal.valueOf(cycles));
	...
	public long calculateMillisFor(MPPOrderNode node, long commonBase)
	{
		...
		long totalDuration =
				+ node.getQueuingTime() 
				+ node.getSetupTime() 		//Ferry + node.getSetupTimeRequiered() // Use the present required setup time to notice later changes
				+ node.getMovingTime() 
				+ node.getWaitingTime()
		;
	...
	private MResourceAssignment createResourceAssign(MPPMRP mrp,
		...
		ra.setAssignDateTo(
				TimeUtil.addMinutess(startAssignTime, duration.setScale(0, RoundingMode.HALF_UP).intValue()));
