...
public class StorageEngine 
{
	...
	{	
		...
		if (product != null && product.isStocked())
		{
			...
			// 查询大于0的库存记录
			MStorageOnHand[] storages = MStorageOnHand.getWarehouse(docLine.getCtx(), M_Warehouse_ID, docLine.getM_Product_ID(), 0,
					null, MClient.MMPOLICY_FiFo.equals(product.getMMPolicy()), true, docLine.getM_Locator_ID(), docLine.get_TrxName());
			
			// 获取第一个库存记录的物料策略日期
			Timestamp dateMPolicy = Optional.ofNullable(storages)
					.filter(arr -> arr.length > 0)
					.map(arr -> arr[0])
					.map(MStorageOnHand::getDateMaterialPolicy)
					.orElse(null);
			...
			if (docLine.getM_AttributeSetInstance_ID() == 0)
			{
				...
				{  
				    ...
				    // ↓ 按索引 j 取对应的 dateMPolicy，不再固定用 storages[0]  
				    Timestamp maDateMPolicy  = (storages != null && j < storages.length)  
				            ? storages[j].getDateMaterialPolicy()  
				            : (storages != null && storages.length > 0 ? storages[0].getDateMaterialPolicy() : null);  
				  
				    if (!MStorageOnHand.add(docLine.getCtx(),  
				            docLine.getM_Locator_ID(),  
				            docLine.getM_Product_ID(),  
				            ma.getM_AttributeSetInstance_ID(),  
				            QtyMA,  
				            maDateMPolicy ,   // ← 现在是对应那条记录的日期  
				            null,  
				            docLine.get_TrxName()))  
				    {  
