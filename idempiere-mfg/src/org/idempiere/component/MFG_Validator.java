...
import org.libero.helper.SubcontractPPOrderHelper;
import org.libero.helper.WFNodeStartNodeHandler;
...
public class MFG_Validator extends AbstractEventHandler {
	...
	protected void initialize() {
		...
		// 新增：注册 AD_WF_Node 表的事件
		registerTableEvent(IEventTopics.PO_AFTER_NEW, MWFNode.Table_Name);
		registerTableEvent(IEventTopics.PO_AFTER_CHANGE, MWFNode.Table_Name);
	...
	protected void doHandleEvent(Event event) {
		...
		// 
			else if (po instanceof MOrderLine && isChange)
			{
				...
				SubcontractPPOrderHelper subcontractPPOrderHelper = new SubcontractPPOrderHelper();
				try {
					subcontractPPOrderHelper.updateOrderDeliveredStatus(order, po.get_TrxName());
				} catch(Exception e) {
			...
			// Create/Update a planning supply when isPurchase Order or you change relevant fields
				if (!order.isSOTrx())
				{
					...
					try {
						subcontractPPOrderHelper.updateSubcontractPPOrder(ol, po.get_TrxName());
					} catch(Exception e) {
		...
		//
			...
			// 新增：处理 AD_WF_Node 事件
					...
					&& (IEventTopics.PO_AFTER_NEW.equals(type) || IEventTopics.PO_AFTER_CHANGE.equals(type))) {
				WFNodeStartNodeHandler.renumberAndUpdateStartNode((MWFNode) po);
			}
		...
		if (po instanceof MInOut && type == IEventTopics.PO_AFTER_NEW) {
			logEvent(event, po, type);//
			MInOut inout = (MInOut)po;
		    if (type == IEventTopics.PO_AFTER_NEW) {  
		        // 处理新创建的M_InOut  
		    	SubcontractPPOrderHelper receiptLinesHelper = new SubcontractPPOrderHelper();
		    	//receiptLinesHelper.autoCreateInOutLines(inout, po.get_TrxName());
		    } 
		}
